...
ohci_pci_probe (struct pci_dev *dev, const struct pci_device_id *id)
{
...
#if 1   /* should add CONFIG_XXXX for embedded OHCI */
    /* add by jedy for embedded OHCI */
    if (PCI_SLOT(dev->devfn) == 5 && PCI_FUNC(dev->devfn) == 0)
        mem_base = PL_USB11_HOST_BASE;
    else
#endif
	...
	if (status < 0) {
		/* iounmap (mem_base); */   /* remove by jedy, it had been released on usb-ohci.c::hc_release_ohci() */
		release_mem_region (mem_resource, mem_len);
		pci_disable_device (dev);
	}
	return status;
