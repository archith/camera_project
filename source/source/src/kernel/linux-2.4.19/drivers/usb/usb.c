...
extern int Host_Disconnect_for_OTG(struct usb_bus *bus, unsigned port_num);
...
void usb_deregister_bus(struct usb_bus *bus)
{
    ...
    if (bus->busnum >= 0)   /* add by jedy for check unregister bus number */
...
//ivan
//printk("usb_internal_control_msg(devnum:%d)\n",usb_dev->devnum);
	urb = usb_alloc_urb(0);
	if (!urb)
		return -ENOMEM;
...
int usb_new_device(struct usb_device *dev)
{
    ...
    dev->bus->b_hnp_enable=0;//Faraday-EHCI(FOTG2XX)
    ...
    info(">>> usb_get_device_descriptor \n");
...
//Start;;Faraday-EHCI(FOTG2XX)
	 ...
	 */
	if (!dev->bus->is_b_host                      //==>Make sure this is not b-host
			&& dev->config                        //==>Make sure this device is config ok
			&& dev->parent == dev->bus->root_hub) //==>Make sure this device is inthe root hub
		{
		...
		if (__usb_get_extra_descriptor (dev->rawdescriptors[0],
					le16_to_cpu(dev->config->wTotalLength),
					USB_DT_OTG, (void **) &desc) == 0) {
			if (desc->bmAttributes & USB_OTG_HNP) {              //==>Check HNP
				unsigned		port1;
				struct usb_device	*root = dev->parent;

				for (port1 = 1; port1 <= root->maxchild;
						port1++) {
					if (root->children[port1-1] == dev)
						break;
				}

				info(">>>Dual-Role OTG device on %sHNP port\n",
					(port1 == bus->otg_port)
						? "" : "non-");

				/* enable HNP before suspend, it's simpler */

					//Temp;;bus->b_hnp_enable = 1;
                             info(">>> USB_REQ_SET_FEATURE \n");

#if 1
                        if (dev->bus->A_Disable_Set_Feature_HNP==0)//The secondary Enumeration => Don't issue HNP Enable
			   {
			   	dev->bus->b_hnp_enable=1;
				err = usb_control_msg(dev,                     //Send set feature
					usb_sndctrlpipe(dev, 0),
					USB_REQ_SET_FEATURE, 0,
					bus->b_hnp_enable
						? USB_DEVICE_B_HNP_ENABLE
						: USB_DEVICE_A_ALT_HNP_SUPPORT,
					0, NULL, 0, HZ * GET_TIMEOUT);
				if (err < 0) {
...
//Start;;Faraday-EHCI(FOTG2XX)
    ...
    if (!is_targeted(dev))
	{
		 ...
		 */
	printk("??? Target is not support in OTG White List...\n");
        //clear_bit(dev->devnum, &dev->bus->devmap.devicemap);
	//dev->devnum = -1;

		if (dev->bus->is_b_host)
		{
		   //B-Host enter the Suspend
                     clear_bit(dev->devnum, &dev->bus->devmap.devicemap);
	             dev->devnum = -1;
		     Host_Disconnect_for_OTG(dev->bus, 0);
		     return 0;
		}
	//return 0;
