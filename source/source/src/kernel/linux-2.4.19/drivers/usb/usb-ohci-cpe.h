#ifndef USB_OHCI_CPE_H
#define USB_OHCI_CPE_H
...
#define ED_NEW 		0x00					// ED 尚未初始化
#define ED_UNLINK 	0x01					// ED 已初始化, 但尚未開始 operate
#define ED_OPER		0x02
#define ED_DEL		0x04
...
struct ohci_regs {
	...
	__u32	fmremaining;
	__u32	fmnumber;
	// missing registers in faraday IP
#if defined(CONFIG_BAD_FARADAY_USB)	
	//__u32	periodicstart;
	//__u32	lsthresh;
	// since non-contiguous register address, some reserved fields must add for faraday IP
	__u32	reserved1[5];				
	/* Root hub ports */
	// faraday IP supports PIO mode, unuseful in SL2312
	__u32	fifodataport;
	__u32	reserved2[3];
#endif
