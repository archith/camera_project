...
#define DBG_TEMP_udc(fmt,args...) \
    printk(KERN_INFO "%s : " fmt , "fsg" , ## args)//Bruce;;Remove
...
//*******************************************************************
// Name:FIFO_Int
// Description:FUSB220 to enable/disable FIFO of dedicated EP
//*******************************************************************
...
//		mval = 1<<(u8fifo_n*2) | 1<<((u8fifo_n+1)*2);		// YPING fixed
...
#warning Using pci_alloc_consistent even with buffers smaller than a page.
		retval = pci_alloc_consistent(ep->dev, bytes, dma);
#endif
	return retval;
}
...
FTC_free_buffer(struct usb_ep *_ep, void *buf, dma_addr_t dma, unsigned bytes)
{
	 ...
	 */
	pci_free_consistent(ep->dev, bytes, buf, dma);
#else
	DBG_FUNCC("+FTC_free_buffer()\n");
	kfree (buf);
#endif
...
write_fifo_top:
	...
	length = min(req->req.length - req->req.actual, ep->ep.maxpacket);
...
//	DBG_TEMP_udc("+start_dma, start addr= 0x%x,total len=%d, Cur len=%d, Dir=%s\n",
//		start,req->req.length,req->u32DMACurrTxLen,(ep->is_in)?"IN":"OUT");
...
//		output: none
///////////////////////////////////////////////////////////////////////////////
static void vCheckDMA(struct FTC_udc *dev)
{
...
static int FTC_usb_probe(void)
{
    ...
    *(volatile u32 *)(CPE_PMU_VA_BASE+0xc8)= (0x8|the_controller->ahb_dma->channel);
	...
	if (request_irq(IRQ_CPE_AHB_DMA,dma_interrupt_handler,SA_INTERRUPT, 
...
//		goto done;
	...
	if (request_irq(VIRQ_USBDEV_AHB_DMA,dma_interrupt_handler,SA_SHIRQ, 
		            driver_name,the_controller) != 0)
	{
		WARN(dev,"Unable to allocate AHB_DMA IRQ %d, maybe already allocated.\n",VIRQ_USBDEV_AHB_DMA);
	}
