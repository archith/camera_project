...
// Description: fill qTD
// Input: <1>.qtd:qTD Structure 
//        <2>.buf:dma buffer address
//        <3>.len:total length
...
qtd_fill (struct ehci_qtd *qtd, dma_addr_t buf, size_t len, int token)
{
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> qtd_fill function \n");
...
//=====================================================================
// Function Name: qh_update
// Description: Update the qh
// Input: <1>.qh:ehci_qh Structure 
//        <2>.qtd:ehci_qtd Structure 
//=====================================================================
static inline void qh_update (struct ehci_qh *qh, struct ehci_qtd *qtd)
{
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> qh_update function \n");

	qh->hw_current = 0;
	qh->hw_qtd_next = QTD_NEXT (qtd->qtd_dma);
	qh->hw_alt_next = EHCI_LIST_END; /* "null pointer" to hw */
...
/*-------------------------------------------------------------------------*/
//=====================================================================
// Function Name: qtd_copy_status
// Description: Copy the Status from token to urb
// Input: <1>.urb:struct urb * 
//        <2>.length:size_t
//        <3>.token:u32
//=====================================================================
static inline void qtd_copy_status (struct urb *urb, size_t length, u32 token)
{
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> qtd_copy_status function \n");
...
//=====================================================================
// Function Name:ehci_urb_complete 
// Description: 
// Input: <1>.
//=====================================================================
static void ehci_urb_complete (
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> ehci_urb_complete function \n");
...
static void ehci_urb_done (
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> ehci_urb_done function \n");
...
#ifdef CONFIG_FARADAY_FEHCI//John_Update
    int         orig_freeing;
	orig_freeing = freeing;
#endif

  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> qh_completions function \n");
...
static void qtd_list_free (
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> qtd_list_free function \n");
...
//=====================================================================
// Function Name: qh_urb_transaction
// Description: 
// Input: <1>.
//=====================================================================
...
qh_urb_transaction (
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> qh_urb_transaction function \n");
...
{
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> clear_toggle function \n");
...
ehci_qh_make (
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> ehci_qh_make function \n");
	...
	case USB_SPEED_LOW:
         printk("### Code Path = Low Speed (Use TT)\n");
		info1 |= (1 << 12);	/* EPS "low" */
		/* FALL THROUGH */

	case USB_SPEED_FULL:
		/* EPS 0 means "full" */
         printk("### Code Path = Full Speed(Use TT) \n");
...
submit_async (
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> submit_async function \n");
...
/*-------------------------------------------------------------------------*/
//=====================================================================
// Function Name: end_unlink_async
// Description: 
// Input: <1>.
//=====================================================================
...
static void end_unlink_async (struct ehci_hcd *ehci)
{
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> end_unlink_async function \n");

  DBG_HOST_EHCI("### Debug => qh=0x%x  \n",qh);
  DBG_HOST_EHCI("### Debug => QH_STATE_IDLE=0x%x  \n",QH_STATE_IDLE);
  DBG_HOST_EHCI("### Debug => qh->qh_state=0x%x  \n",qh->qh_state);
  
	qh->qh_state = QH_STATE_IDLE;

  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> end_unlink_async function (qh->qh_state = QH_STATE_IDLE;)\n");
...
//=====================================================================
// Function Name: start_unlink_async
// Description: 
// Input: <1>.
//=====================================================================
...
static void start_unlink_async (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> start_unlink_async function \n");
...
// this macro lies except on SMP compiles
	...
	if (unlikely (qh == ehci->async && qh->qh_next.qh == qh)) {
		...
		if (ehci->hcd.state != USB_STATE_HALT) {
			...
			//bruce;;04292005;;	while (readl (&ehci->regs->status) & STS_ASS)
					udelay (100);
			}
		}
...
static void scan_async (struct ehci_hcd *ehci)
{
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-q.c file --> scan_async function \n");
