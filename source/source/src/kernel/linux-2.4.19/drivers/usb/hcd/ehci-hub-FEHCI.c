...
static int check_reset_complete (
 ...
 //Timing issue;; DBG_HOST_EHCI("### >>> Enter ehci-hub.c file --> check_reset_complete function \n");
...
ehci_hub_status_data (struct usb_hcd *hcd, char *buf)
{
  ...
  //DBG_HOST_EHCI("### >>> Enter ehci-hub.c file --> ehci_hub_status_data function \n");
...
//<2>.If not-connect => clear the reset-array bit 
...
ehci_hub_descriptor (
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-hub.c file --> ehci_hub_descriptor function \n");
...
static int ehci_hub_control (
  ...
  DBG_HOST_EHCI("### >>> Enter ehci-hub.c file --> ehci_hub_control function \n");
 ...
 //---
	case ClearPortFeature:
		...
		case USB_PORT_FEAT_ENABLE:
			////For FUSBH200;;writel (temp & ~PORT_PE,&ehci->regs->port_status [wIndex]);
...
#else
		case USB_PORT_FEAT_POWER://John Remove
			if (HCS_PPC (ehci->hcs_params))
...
#ifdef CONFIG_FARADAY_FEHCI//John_Update
#else
		case USB_PORT_FEAT_C_OVER_CURRENT://John Remove
 ...
 //---
	case GetPortStatus:
		...
		if ((temp & PORT_RESET)
			...
			status |= 1 << USB_PORT_FEAT_C_RESET;
                    ...
                    DBG_HOST_TEMP("### ehci-hub.c-->ehci_hub_control()-->if ((temp & PORT_RESET) status=0x%x...\n",status);    
...
#ifdef CONFIG_FARADAY_FEHCI//John_Update
#else
			if (temp & PORT_POWER)//John Remove
	...
	if (status & ~0xffff)	/* only if wPortChange is interesting */
		...
		*((u32 *) buf) = cpu_to_le32 (status);
                ...
                DBG_HOST_TEMP("### ehci-hub.c-->ehci_hub_control()-->GetStatus=0x%x...\n",status);    
