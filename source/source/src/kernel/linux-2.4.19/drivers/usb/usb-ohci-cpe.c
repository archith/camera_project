...
#define DEBUG
//ivan
#define dbg(args...)	printk("DEBUG:" args "\n");
 ...
 *-------------------------------------------------------------------------*/ 
static void td_list_print(char *str, ohci_t * ohci, __u32 td_dma)
{
	volatile td_t  * td;
	
	//while (td_dma != 0) {
		td = dma_to_td (ohci, td_dma);
		printk("%s: td_dma = %08x ", str, td_dma);
		printk("hwINFO = %08x ", td->hwINFO);
		printk("hwCBP = %08x ", td->hwCBP);
		printk("hwBE = %08x ", td->hwBE);
		printk("hwNextTD = %08x\n", td->hwNextTD);		
		td_dma = td->hwNextTD;
	//}
}
...
static int sohci_return_urb (struct ohci *hc, urb_t * urb)
{
	...
	//td_list_print("sohci_return_urb", hc, urb_priv->td [0]->td_dma);
	//td_list_print("sohci_return_urb", hc, urb_priv->td [1]->td_dma);
	//td_list_print("sohci_return_urb", hc, urb_priv->td [2]->td_dma);
...
//ivan
printk("sohci_submit_urb(devnum=%d)\n",urb->dev->devnum);	
	if (!urb->dev || !urb->dev->bus)
		return -ENODEV;
...
//		return -EPIPE;
	...
	switch (usb_pipetype (pipe)) {
		case PIPE_BULK:	/* one TD for every 4096 Byte */
			size = (urb->transfer_buffer_length - 1) / 4096 + 1;				// size ==> 需要 size 個 TD
	...
	switch (usb_pipetype (pipe)) {
		...
		case PIPE_INTERRUPT:
			if (urb->bandwidth == 0) {
				bustime = usb_check_bandwidth (urb->dev, urb);			/// 算出所需的 bustime
			}
			if (bustime < 0) {
				urb_free_priv (ohci, urb_priv);
				spin_unlock_irqrestore (&usb_ed_lock, flags);
...
)
{
	...
	ed->hwINFO = cpu_to_le32 (usb_pipedevice (pipe)								// FunctionAddress
			| usb_pipeendpoint (pipe) << 7										// EndpointNumber
			| (usb_pipeisoc (pipe)? 0x8000: 0)									// Format: 0==>general TD, 1 ==> iso TD
			| (usb_pipecontrol (pipe)? 0: (usb_pipeout (pipe)? 0x800: 0x1000)) 	// Direction
			| usb_pipeslow (pipe) << 13											// Speed: 0 ==> full-speed, 1 ==> low-speed
...
{    
	...
	if (!ohci->disabled && !ohci->sleeping) {
		...
		writel (readl(&ohci->regs->intrenable) | OHCI_INTR_SF, &ohci->regs->intrenable);
	}
}
...
//if (data & (1 << 20)) panic("td_fill: A20 = 1: %08x\n", data);
	...
	//printk("ed: hwINFO = %08x, hwHeadP = %08x, hwTailP = %08x\n", td->ed->hwINFO, td->ed->hwHeadP, td->ed->hwTailP);
	//printk("td_fill: td = %08x, INFO = %08x, CBP = %08x, NextTD = %08x\n", td->td_dma, td->hwINFO, td->hwCBP, td->hwNextTD);
}
...
static int hc_reset (ohci_t * ohci)
{
	...
	writel (readl(&ohci->regs->intrenable) & ~OHCI_INTR_MIE, &ohci->regs->intrenable);
...
static int hc_reset (ohci_t * ohci)
{
	...
	writel(readl(&ohci->regs->cmdstatus) | OHCI_HCR, &ohci->regs->cmdstatus);
	while (readl(&ohci->regs->cmdstatus) & OHCI_HCR);
	printk("End of USB reset\n");
	lvalue = readl(&ohci->regs->control);
	lvalue &= ~(OHCI_CTRL_HCFS);
	lvalue |= OHCI_USB_RESET;
	writel(lvalue, &ohci->regs->control);
...
static int hc_start (ohci_t * ohci)
{
	...
	writel (ohci->hcca_dma, &ohci->regs->hcca); /* a reset clears this */
  	...
  	fminterval = 0x2edf;
#if !defined(CONFIG_BAD_FARADAY_USB)
...
static void hc_interrupt (int irq, void * __ohci, struct pt_regs * r)
{
 ...
 info("SL2312 interrupt");
	...
	if (ints & OHCI_INTR_WDH) {
		// HcInterruptEnable of Faraday is different from standard OHCI
		writel (readl(&regs->intrenable) & ~OHCI_INTR_WDH, &regs->intrenable);	
	...
	if (ints & OHCI_INTR_SF) { 
		...
		writel (readl(&regs->intrenable) & ~OHCI_INTR_SF, &regs->intrenable);
...
// On unloaded USB, think 4k ~= 4-5msec
	...
	writel (readl(&regs->intrenable) | OHCI_INTR_MIE, &regs->intrenable);	
}
...
void usb_dump_priv(urb_t *urb)
{
	
	urb_priv_t * urb_priv = urb->hcpriv;
	//struct ohci_regs * regs = cpe_ohci->regs;
	td_t *td[3];
	...
	printk("%x, %x, %x, %x\n", urb_priv, td[0], td[1], td[2]);
	//info("usb_dump_priv: source = %08x, usb irq status = %08x, enable = %08x", *((volatile unsigned int *)IRQ_SOURCE(IO_ADDRESS(CPE_IC_BASE))), readl (&regs->intrstatus), readl (&regs->intrenable));
 	//info("usb_dump_priv: mask = %08x", *((volatile unsigned int *)IRQ_MASK(IO_ADDRESS(CPE_IC_BASE))));				
}
#endif /* end_of_not */
...
//static ohci_t *cpe_ohci;


static void __init cpe_ohci_configure(void)
{
	fLib_SetIntTrig(IRQ_USB, LEVEL, H_ACTIVE);
}


static int __init cpe_ohci_init(void)
{
	void *mem_base;
	int ret;
...
//init ahb decoder(ivan)
	*(unsigned int *)(IO_ADDRESS(CPE_DECODER_BASE)+0x44)=CPE_USB_BASE;
        mdelay(100);	

	cpe_ohci_configure();

	mem_base = IO_ADDRESS(CPE_USB_BASE);

	/*
	 * Initialise the generic OHCI driver.
	 */
	ret = hc_add_ohci((struct pci_dev *)1, IRQ_USB,
			  (void *) mem_base, 0, &cpe_ohci,
			  "usb-ohci", "sl2312");
...
static void __exit cpe_ohci_exit(void)
{
	hc_remove_ohci(cpe_ohci);
...
module_init(cpe_ohci_init);
module_exit(cpe_ohci_exit);
