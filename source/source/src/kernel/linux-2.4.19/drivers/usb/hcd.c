...
#include "FTC_Debug.h"//Faraday-EHCI(FOTG2XX)
...
static int ascii2utf (char *s, u8 *utf, int utfmax)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> ascii2utf function \n");
...
static int rh_string (
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> rh_string function \n");
...
static int rh_call_control (struct usb_hcd *hcd, struct urb *urb)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> rh_call_control function \n");
...
static int rh_status_urb (struct usb_hcd *hcd, struct urb *urb)
{
  ...
  //DBG_HOST_HCD("### >>> Enter hcd.c file --> rh_status_urb function \n");

//Faraday-EHCI(FOTG2XX)
  if (hcd!=ghcd_FOTG2XX)//Faraday-EHCI(FOTG2XX);;Remove the Checking condition
     {	/* rh_timer protected by hcd_data_lock */
	if (timer_pending (&hcd->rh_timer)
			|| urb->status != -EINPROGRESS
			|| !HCD_IS_RUNNING (hcd->state)
			|| urb->transfer_buffer_length < len) {
		DBG_HOST_TEMP ("not queuing status urb, stat %d", urb->status);
		return -EINVAL;
	}
     }
...
static void rh_report_status (unsigned long ptr)
{
  ...
  //DBG_HOST_HCD("### >>> Enter hcd.c file --> rh_report_status function \n");
...
#if 0
//Start;;Faraday-EHCI(FOTG2XX);;IF A_HNP enable => Only restart the timer
  if (hcd==ghcd_FOTG2XX)
     { if (mdwHostPort(0x80)&BIT7)  //Checking for A_HNP enable
         {
...
//End;;Faraday-EHCI(FOTG2XX);;IF A_HNP enable => Only restart the timer
#endif
...
//<1>.call hub_status_data() function
...
//<2>.call rh_status_urb() function
		...
		} else
			{
			    ...
			    rh_status_urb (hcd, urb);//Faraday-EHCI(FOTG2XX)
			spin_unlock_irqrestore (&urb->lock, flags);
			}
	} else {
		/* this urb's been unlinked */
  	        DBG_HOST_TEMP("??? rh_report_status:urb->status != -EINPROGRESS (urb->status=%d)(hcd->state=%d) \n",urb->status,hcd->state);
...
{
  DBG_HOST_HCD("### >>> Enter hcd.c file --> rh_urb_enqueue function \n");
...
static void rh_status_dequeue (struct usb_hcd *hcd, struct urb *urb)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> rh_status_dequeue function \n");
...
void usb_ehcd_FEHCI_probe (struct hc_driver	*driver,int iType) //Input the EHCI Driver pointer
{
	struct usb_bus		*bus;
	struct usb_hcd		*hcd;
    unsigned int	ioaddr ;
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> usb_ehcd_FUSB200_probe function (iType=%d)\n",iType);
  ...
  if (iType>1)
	...
	cpe_int_set_irq(IRQ_USB_HOST20 ,LEVEL,H_ACTIVE);  //Use IRQ 29

      //Step2:Allocate & Init HCD
        DBG_HOST_HCD("###  //<2.1>Allocate ehci_HCD & HCD\n");
        ...
        //<2.2>.Set Data to HCD
               ...
               DBG_HOST_HCD("###%s \n", hcd->description);
	           DBG_HOST_HCD("### hcd->irq = 0x%x\n",hcd->irq);
	           DBG_HOST_HCD("### hcd->regs = 0x%x\n",hcd->regs);
	           ioaddr = (unsigned int)hcd->regs;
               DBG_HOST_HCD("### Offset 0x80=0x%x\n",inl( ioaddr + 0x80 ));

        //<2.3>.Allocate Bus
	           bus = usb_alloc_bus (&hcd_operations);
	           if (bus == NULL) {
	               DBG_HOST_HCD("EEE Allocate Bus Fail\n");
	           }
	           hcd->bus = bus;
	           bus->hcpriv = (void *) hcd;
...
int usb_hcd_pci_probe (struct pci_dev *dev, const struct pci_device_id *id)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> usb_hcd_pci_probe function \n");

	if (!id || !(driver = (struct hc_driver *) id->driver_data))
		return -EINVAL;

 //<1>.Enable PCI Devices
...
void usb_hcd_pci_remove (struct pci_dev *dev)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> usb_hcd_pci_remove function \n");
...
int usb_hcd_pci_suspend (struct pci_dev *dev, u32 state)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> usb_hcd_pci_suspend function \n");
...
int usb_hcd_pci_resume (struct pci_dev *dev)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> usb_hcd_pci_resume function \n");
...
static int hcd_alloc_dev (struct usb_device *udev)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> hcd_alloc_dev function \n");
...
static void hc_died (struct usb_hcd *hcd)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> hc_died function \n");
...
static void urb_unlink (struct urb *urb)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> urb_unlink function \n");
...
static int hcd_submit_urb (struct urb *urb)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> hcd_submit_urb function \n");
...
//Faraday-EHCI(FOTG2XX/FUSBH200)
                if ((hcd==ghcd_FOTG2XX)|(hcd==ghcd_FUSBH200))//Faraday-EHCI(FOTG2XX/FUSBH200)
    		    urb->interval = urb->interval * 8;  //john enlarge to us
...
static int hcd_get_frame_number (struct usb_device *udev)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> hcd_get_frame_number function \n");
...
static void unlink_complete (struct urb *urb)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> unlink_complete function \n");
...
static int hcd_unlink_urb (struct urb *urb)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> hcd_unlink_urb function \n");
...
static int hcd_free_dev (struct usb_device *udev)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> hcd_free_dev function \n");
...
static int hcd_irq_For_OTG (struct usb_bus	*bus)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> hcd_irq_For_OTG function \n");


    hcd=(struct usb_hcd		*)(bus->hcpriv);
    start = hcd->state;
	if (unlikely (ghcd_FOTG2XX->state == USB_STATE_HALT))	/* irq sharing? */
		return 0;

	ghcd_FOTG2XX->driver->irq (ghcd_FOTG2XX);
	if (ghcd_FOTG2XX->state != start && ghcd_FOTG2XX->state == USB_STATE_HALT)
		hc_died (ghcd_FOTG2XX);

  return 0;
}

EXPORT_SYMBOL (hcd_irq_For_OTG);
//End;;Faraday-EHCI(FOTG2XX)
...
void usb_hcd_giveback_urb (struct usb_hcd *hcd, struct urb *urb)
{
  ...
  DBG_HOST_HCD("### >>> Enter hcd.c file --> usb_hcd_giveback_urb function \n");
