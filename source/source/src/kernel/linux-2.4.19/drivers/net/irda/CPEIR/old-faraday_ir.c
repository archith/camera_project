...
static void ali_ircc_sir_interrupt(int irq, struct ali_ircc_cb *self, struct pt_regs *regs)
{
	...
	if (iir) {	
		...
		switch (iir) 
		{
			//case UART_IIR_RLSI:
			case SERIAL_IIR_RLS:
				P_DEBUG( __FUNCTION__ "(), SERIAL_IIR_RLS\n");
				break;
			//case UART_IIR_RDI:
			case SERIAL_IIR_DR:
				/* Receive interrupt */
...
static void ali_ircc_sir_write_wakeup(struct ali_ircc_cb *self)
{
	...
	P_DEBUG( __FUNCTION__ "(), ---------------- Start ---------------- self->tx_buff.len=0x%X\n",self->tx_buff.len);
	...
	else 
	{
		if (self->new_speed) 
		{
			...
			while(!(INW(iobase+SERIAL_LSR) & SERIAL_LSR_TE))
				P_DEBUG( __FUNCTION__ "(), UART_LSR_THRE\n");
			
			P_DEBUG( __FUNCTION__ "(), Changing speed! self->new_speed = %d\n", self->new_speed);
			...
			if (self->io.speed > 115200)
			{
				P_DEBUG( __FUNCTION__ "(), ali_ircc_change_speed from UART_LSR_TEMT \n");				
...
static void ali_ircc_sir_change_speed(struct ali_ircc_cb *priv, __u32 speed)
{
	...
	P_DEBUG( __FUNCTION__ "(), Setting speed to: %d\n", speed);
...
static int ali_ircc_net_open(struct net_device *dev)
{
	 ...
	 * failure.
	 */
	if (fapb_request_dma(self->io.dma, dev->name)) {
