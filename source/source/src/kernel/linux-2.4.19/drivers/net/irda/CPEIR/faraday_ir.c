//#define FARADAY_IRDA_PM
//#define FARADAY_IRDA_FIR
...
#include <net/irda/faraday_ir.h>
#include "cpe_serial.h"
#include "irda.h"

#define CHIP_IO_EXTENT 68
//#define BROKEN_DONGLE_ID

static char *driver_name = "Faraday_IRDA";
...
/* FIR function */
	...
	static int  ali_ircc_fir_hard_xmit(struct sk_buff *skb, struct net_device *dev)
		{
			printk("ali_ircc_fir_hard_xmit()\n");
			return -1;
		}
	static void ali_ircc_fir_change_speed(struct ali_ircc_cb *priv, __u32 speed)
		{
			printk("ali_ircc_fir_change_speed()\n");
		}
	static void ali_ircc_fir_interrupt(int irq, struct ali_ircc_cb *self, struct pt_regs *regs)
		{
			printk("ali_ircc_fir_interrupt()\n");
		}	
#endif	
...
static int ali_ircc_setup(chipio_t *info)
{
	...
	P_DEBUG( __FUNCTION__ "(), ---------------- Start ----------------\n");

#ifdef FARADAY_IRDA_FIR	
#else
	fLib_SetSIRSpeed(iobase,9600);
	fLib_SetIRMode(iobase,SERIAL_MDR_SIR);
	fLib_SetFIFO(iobase);	
		
	 Sir_Disable_TxRx(iobase);
#endif	 
	/* Switch to FIR space */
	//SIR2FIR(iobase);
...
//		ERROR("%s, Wrong chip version %02x\n", driver_name, version);
	...
	MESSAGE("%s, driver loaded (Paul Chiang)\n", driver_name);
	
	
	P_DEBUG( __FUNCTION__ "(), ----------------- End ------------------\n");	
...
/*#ifdef FARADAY_IRDA_PM		
	...
	/* Set FIR FIFO and DMA Threshold */
	if ((ali_ircc_setup(info)) == -1)	//9600 8N1
		return -1;
    ...
    self->io.irq2       = info->irq2;
    self->io.fir_ext   = CHIP_IO_EXTENT;
    self->io.dma       = info->dma;
    //self->io.fifo_size = 16;		/* SIR: 16, FIR: 32 Benjamin 2000/11/1 */
    self->io.fifo_size = 64;		
#ifdef FARADAY_IRDA_FIR	
	self->io.speed=4000000;//20030902
#else    
	self->io.speed=9600;//20030902
#endif	
	/* Reserve the ioports that we need */
	//if (!request_region(self->io.fir_base, self->io.fir_ext, driver_name)) {
	if (!REQUEST_IO_REGION(self->io.fir_base, self->io.fir_ext, driver_name)) {
	...
	self->qos.baud_rate.bits = IR_9600|IR_19200|IR_38400|IR_57600|
		IR_115200|(IR_4000000 << 8); // benjamin 2000/11/8 05:27PM
	...
	self->flags = IFF_FIR|IFF_SIR|IFF_DMA|IFF_PIO; 	// benjamin 2000/11/8 05:27PM	

	/* Max DMA buffer size needed = (data_size + 6) * (window_size) + 6; */
	//self->rx_buff.truesize = 14384; 
	//self->tx_buff.truesize = 14384;
	self->rx_buff.truesize = RX_BUFF_SIZE; 	//paulong test test
	self->tx_buff.truesize = TX_BUFF_SIZE;
...
static int ali_ircc_read_dongle_id (int i, chipio_t *info)
{
	...
	P_DEBUG( __FUNCTION__ "(), probing dongle_id=%d, dongle_types=%s\n", 
...
static void ali_ircc_fir_interrupt(int irq, struct ali_ircc_cb *self, struct pt_regs *regs)
{
	...
	P_DEBUG( __FUNCTION__ "(), self->InterruptID = %x\n",self->InterruptID);
	P_DEBUG( __FUNCTION__ "(), self->LineStatus = %x\n",self->LineStatus);
	P_DEBUG( __FUNCTION__ "(), self->ier = %x\n",self->ier);
	P_DEBUG( __FUNCTION__ "(), eir = %x\n",eir);
	...
	{		
		...
		{
			P_DEBUG( __FUNCTION__ "(), ******* IIR_EOM (Tx) *******\n");
		...
		{
			P_DEBUG( __FUNCTION__ "(), ******* IIR_EOM (Rx) *******\n");
			
			if(OldMessageCount > ((self->LineStatus+1) & 0x07))
			{
				self->rcvFramesOverflow = TRUE;	
				P_DEBUG( __FUNCTION__ "(), ******* self->rcvFramesOverflow = TRUE ******** \n");
			}
						
			if (ali_ircc_dma_receive_complete(self))
			{
				P_DEBUG( __FUNCTION__ "(), ******* receive complete ******** \n");
				
				self->ier = IER_EOM;				
			}
			else
			{
				P_DEBUG( __FUNCTION__ "(), ******* Not receive complete ******** \n");
...
static void ali_ircc_sir_interrupt(int irq, struct ali_ircc_cb *self, struct pt_regs *regs)
{
	...
	P_DEBUG_SIR( __FUNCTION__ "(), ---------------- Start ----------------\n");
	
	iobase = self->io.sir_base;

	//iir = inb(iobase+UART_IIR) & UART_IIR_ID;
	iir = fLib_SerialIntIdentification(iobase) & SERIAL_IIR_ID;
	if (iir) {	
		/* Clear interrupt */
		//lsr = inb(iobase+UART_LSR);
		lsr=INW(iobase + SERIAL_LSR);

		P_DEBUG_SIR( __FUNCTION__ 
		...
		switch (iir) 
		{
			//case UART_IIR_RLSI:
			case SERIAL_IIR_RLS:
				P_DEBUG_SIR( __FUNCTION__ "(), SERIAL_IIR_RLS\n");
				break;
			//case UART_IIR_RDI:
			case SERIAL_IIR_DR:
				/* Receive interrupt */
...
static void ali_ircc_sir_receive(struct ali_ircc_cb *self) 
{
	...
	/*do {
		async_unwrap_char(self->netdev, &self->stats, &self->rx_buff, 
				  (u8)INW(iobase + SERIAL_RBR));
		//		  inb(iobase+UART_RX));

		// Make sure we don't stay here to long 
		if (boguscount++ > 32) {
			P_DEBUG_SIR(__FUNCTION__ "(), breaking!\n");
			break;
		}
	//} while (inb(iobase+UART_LSR) & UART_LSR_DR);	
	} while (INW(iobase+SERIAL_LSR) & SERIAL_LSR_DR);	*/
	while (INW(iobase+SERIAL_LSR) & SERIAL_LSR_DR)	//20030902
	   {
		async_unwrap_char(self->netdev, &self->stats, &self->rx_buff, 
				  (u8)INW(iobase + SERIAL_RBR));
		//		  inb(iobase+UART_RX));
	...
	//printk("---------------receive\n");
	 //DumpBuff(self->rx_buff.data,self->rx_buff.len);//test by Chalres Tsai 
#ifdef PAUL_DEBUG		
		DumpBuff(self->rx_buff.data,self->rx_buff.len);
#endif				
...
static void ali_ircc_sir_write_wakeup(struct ali_ircc_cb *self)
{
	...
	P_DEBUG_SIR( __FUNCTION__ "(), ---------------- Start ---------------- self->tx_buff.len=0x%X\n",self->tx_buff.len);
	...
	else 
	{
		if (self->new_speed) 
		{
			...
			while(!(INW(iobase+SERIAL_LSR) & SERIAL_LSR_TE))
				P_DEBUG_SIR( __FUNCTION__ "(), UART_LSR_THRE\n");
			
			P_DEBUG_SIR( __FUNCTION__ "(), Changing speed! self->new_speed = %d\n", self->new_speed);
			printk("write wake up change speed\n");
			...
			if (self->io.speed > 115200)
			{
				P_DEBUG_SIR( __FUNCTION__ "(), ali_ircc_change_speed from UART_LSR_TEMT \n");				
		...
		{
			P_DEBUG_SIR("netif_wake_queue(self->netdev)\n");
		...
		//fLib_SetSerialInt(iobase, SERIAL_IER_DR|SERIAL_IER_RLS); //0901				
		Sir_Enable_Rx(iobase);	//20030902
...
static void ali_ircc_change_speed(struct ali_ircc_cb *self, __u32 baud)
{
	...
	P_DEBUG( __FUNCTION__ "(), setting speed = %d \n", baud);
	...
	P_DEBUG("netif_wake_queue(self->netdev)\n");
	netif_wake_queue(self->netdev);	
...
static void ali_ircc_fir_change_speed(struct ali_ircc_cb *priv, __u32 baud)
{
	...
	P_DEBUG( __FUNCTION__ "(), self->io.speed = %d, change to speed = %d\n",self->io.speed,baud);
...
static void ali_ircc_sir_change_speed(struct ali_ircc_cb *priv, __u32 speed)
{
	...
	P_DEBUG_SIR( __FUNCTION__ "(), Setting speed to: %d\n", speed);
	...
	{	
		...
		//ali_ircc_change_dongle_speed(self, speed);			//move to FIR2SIR()
		FIR2SIR(iobase);
	}
	...
	if(self->io.speed >115200)		//20030902 add
		fLib_SetIRMode(iobase,SERIAL_MDR_FIR);
	else
		fLib_SetIRMode(iobase,SERIAL_MDR_SIR);
	//outb(UART_LCR_DLAB | lcr, iobase+UART_LCR); /* Set DLAB */
	//outb(divisor & 0xff,      iobase+UART_DLL); /* Set speed */
	//outb(divisor >> 8,	  iobase+UART_DLM);
	//outb(lcr,		  iobase+UART_LCR); /* Set 8N1	*/	//move to fLib_SetIRMode()
...
static void ali_ircc_change_dongle_speed(struct ali_ircc_cb *priv, int speed)
{
	...
	P_DEBUG( __FUNCTION__ "(), Set Speed for %s , Speed = %d\n", dongle_types[dongle_id], speed);		
	
	if(speed>115200)	//FIR
		fLib_IrDA_AutoMode_FIR_High(iobase);	
	else	//SIR		
		fLib_IrDA_AutoMode_SIR_Low(iobase);	
...
static int ali_ircc_sir_write(int iobase, int fifo_size, __u8 *buf, int len)
{
	...
	//if (!(INW(iobase + SERIAL_LSR) & SERIAL_LSR_THRE)) {	//20030902 remove
	//	IRDA_DEBUG(0, __FUNCTION__ "() 1, failed, fifo not empty!\n");
	//	return 0;
	//}
	...
	{
		//............20030902 add
		//status=INW(iobase + SERIAL_LSR);
		//status1= *((unsigned int*)0xf9890014);
		...
		do
		 {
			 ...
			 status=INW(iobase + SERIAL_LSR);
			// printk("status:%x\n", status);
			 //printk("iobase:%x, status:%x, lsr_thre:%x, result:%x\n", iobase, status, SERIAL_LSR_THRE, status & SERIAL_LSR_THRE);
			if(timeout++ > SIR_TX_TIMEOUT)
				{
				printf( __FUNCTION__ "() 2, failed, fifo not empty!\n");
				return actual;
				}			
		}
		while (( status & SERIAL_LSR_THRE)!=SERIAL_LSR_THRE);//............20030902 add
		//printk("here\n");
...
static int ali_ircc_net_open(struct net_device *dev)
{
	...
	if (request_irq(self->io.irq2, ali_ircc_interrupt, 0, dev->name, dev)) 	//fir 
	{
		free_irq(self->io.irq, dev);	
		WARNING("%s, unable to allocate irq2=%d\n", driver_name, 
			self->io.irq2);
		return -EAGAIN;
	}
	 ...
	 * failure.
	 */
	if (fapb_request_dma(self->io.dma, dev->name)) {// org is request_dma
		WARNING("%s, unable to allocate dma=%d\n", driver_name, 
			self->io.dma);
		free_irq(self->io.irq, self);
		free_irq(self->io.irq2, self);
		return -EAGAIN;
	}//mark by Charles Tsai
	
	/* Turn on interrups */
	//outb(UART_IER_RLSI | UART_IER_RDI |UART_IER_THRI, iobase+UART_IER);
	//fLib_SetSerialInt(iobase,SERIAL_IER_DR|SERIAL_IER_TE|SERIAL_IER_RLS); test test paulong 20030901
	Sir_Enable_Rx(iobase);//20030902
	//fLib_SetSerialInt(iobase,SERIAL_IER_DR|SERIAL_IER_RLS);
	/* Ready to play! */
	P_DEBUG("netif_start_queue(dev)\n");
...
{	
	...
	fapb_disable_dma(self->io.dma);// mark by Charles Tsai, org is disable_dma
	...
	fapb_free_dma(self->io.dma);// mark by Charles Tsai, org is free_dma
...
static void ali_ircc_dma_xmit(struct ali_ircc_cb *self)
{
	...
	P_DEBUG( __FUNCTION__ "(), ******* Change to TX mode: FIR_LCR_B = 0x%x ******* \n", inb(iobase+FIR_LCR_B));
...
static int  ali_ircc_dma_xmit_complete(struct ali_ircc_cb *self)
{
	...
	if (self->new_speed) 
	{
		printk("dma complete change speed\n");
		ali_ircc_change_speed(self, self->new_speed);
		self->new_speed = 0;
	}
...
static int ali_ircc_dma_receive(struct ali_ircc_cb *self) 
{
	...
	P_DEBUG( __FUNCTION__ "(), *** Change To RX mode: FIR_LCR_B = 0x%x *** \n", inb(iobase+FIR_LCR_B));
...
static int  ali_ircc_dma_receive_complete(struct ali_ircc_cb *self)
{
	...
	for (i=0; i<=MessageCount; i++)
	{
		...
		P_DEBUG( __FUNCTION__ "(), RX Length = 0x%.2x,\n", len);	
		P_DEBUG( __FUNCTION__ "(), RX Status = 0x%.2x,\n", status);
...
static int ali_ircc_sir_hard_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	P_DEBUG_SIR( __FUNCTION__ "(), ---------------- Start ----------------\n");
	P_DEBUG_SIR( __FUNCTION__ "(%ld), skb->len=%d\n", jiffies, (int) skb->len);
	...
	P_DEBUG_SIR("netif_stop_queue(dev)\n");
	netif_stop_queue(dev);
	
	/* Check if we need to change the speed */
	speed = irda_get_next_speed(skb);
	speed = self->io.speed; // test by Charles Tsai
	if ((speed != self->io.speed) && (speed != -1)) {
		/* Check for empty frame */
		if (!skb->len) {
			printk("sir hard change speed\n");
			ali_ircc_change_speed(self, speed); 
			dev_kfree_skb(skb);
			return 0;
		} else
...
#ifdef  PAUL_DEBUG
		DumpBuff(self->tx_buff.data,self->tx_buff.len);
		DumpRegister(iobase+4,iobase+0x68);
#endif		

	/* Turn on transmit finished interrupt. Will fire immediately!  */
	//outb(UART_IER_THRI, iobase+UART_IER); 
	//fLib_SetSerialInt(iobase,SERIAL_IER_TE);	
	Sir_Enable_Tx(iobase);//20030902

	ali_ircc_sir_write_wakeup(self);//20030902
...
static int ali_ircc_net_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	IRDA_DEBUG(0, __FUNCTION__ "(), ---------------- Start ----------------\n");
	...
	IRDA_DEBUG(0, __FUNCTION__ "(), %s, (cmd=0x%X)\n", dev->name, cmd);
	...
	case SIOCSBANDWIDTH: /* Set bandwidth */
		IRDA_DEBUG(0, __FUNCTION__ "(), SIOCSBANDWIDTH\n");
		/*
		...
		printk("ioctl change speed\n");
		ali_ircc_change_speed(self, irq->ifr_baudrate);		
		break;
	case SIOCSMEDIABUSY: /* Set media busy */
		IRDA_DEBUG(0, __FUNCTION__ "(), SIOCSMEDIABUSY\n");
		if (!capable(CAP_NET_ADMIN))
			return -EPERM;
		irda_device_set_media_busy(self->netdev, TRUE);
		break;
	case SIOCGRECEIVING: /* Check if we are receiving right now */
		IRDA_DEBUG(0, __FUNCTION__ "(), SIOCGRECEIVING\n");
...
static int ali_ircc_is_receiving(struct ali_ircc_cb *self)
{
	...
	if (self->io.speed > 115200) 
	{
		...
		if((inb(iobase+FIR_FIFO_FR) & 0x3f) != 0) 		
		{
			...
			P_DEBUG( __FUNCTION__ "(), We are receiving something\n");
			status = TRUE;
		}
		switch_bank(iobase, BANK0);		
	} 
...
static void SetCOMInterrupts(struct ali_ircc_cb *self , unsigned char enable)
{
	...
	P_DEBUG( __FUNCTION__ "(), -------- Start -------- ( Enable = %d )\n", enable);	
	...
	if (enable){
		if (self->io.direction == IO_XMIT)
		{
			...
			else /* SIR */
			{
				...
				newMask = SERIAL_IER_DR | SERIAL_IER_TE|SERIAL_IER_RLS;
			}
		}
		else {
			if (self->io.speed > 115200) /* FIR, MIR */
			...
			else /* SIR */
			{
				...
				newMask = SERIAL_IER_DR |SERIAL_IER_RLS;
			}
		}
	}
	...
	{	
		...
		fLib_SetSerialInt(iobase,newMask);
...
static void FIR2SIR(int iobase)
{
	/*unsigned char val;		20030902 remove
	unsigned long flags;
...
{	
	...
	cmd = INW(CPE_PWM_BASE+0x28);
	cmd |= 0x1004;
	cmd &= 0xfffffdff;
	OUTW(CPE_PWM_BASE+0x28, cmd);
#endif
			...
			if (io[i] !=0)				
					ret=fLib_irda_probe(info.fir_base);	
...
MODULE_DESCRIPTION("Faraday SIR/FIR Controller Driver");
MODULE_LICENSE("GPL");
#endif /* MODULE */
