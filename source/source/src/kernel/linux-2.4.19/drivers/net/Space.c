...
extern int ftmac100_init( struct net_device * );
...
extern int FTCmac_probe(struct net_device *);   //john
...
//john add for 2 ftmac100s
static struct devprobe ftmac_probes[] __initdata = {
#ifdef CONFIG_FTMAC100
	{FTCmac_probe, 0},
#endif
	{NULL, 0},
};
...
static int __init ethif_probe(struct net_device *dev)
{
	...
	if (probe_list(dev, mca_probes) == 0)
		return 0;
	if (probe_list(dev, ftmac_probes) == 0)   //john add for 2 FTMAC100s
		return 0;
        /*
