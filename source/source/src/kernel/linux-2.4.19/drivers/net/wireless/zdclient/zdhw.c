...
//printk("HW_Set_GCT_Chips\n");
	...
	//	 CCA always high.
	pObj->SetReg(reg, ZD_CR33, 0x08);   //john for GCT 0x28);
	//--                  
...
extern int zd1205_mode;
void HW_EnableIBSS(U16 BecaonInterval, U16 DtimPeriod) 
{
	U32 tmpValue;
	void *reg = pdot11Obj->reg;
	zd_80211Obj_t *pObj=pdot11Obj;
	
//	printk("EnableIBSS:0x%x\n",tmpValue);
	tmpValue = BecaonInterval | IBSS_MODE | (DtimPeriod<<16) ;
	pObj->SetReg(reg, ZD_BCNInterval, tmpValue);
	zd1205_mode = INDEPENDENT_BSS; 
}	


void HW_EnableSTA(U16 BecaonInterval, U16 DtimPeriod) 
{
//	U32 tmpValue;
	void *reg = pdot11Obj->reg;
	zd_80211Obj_t *pObj=pdot11Obj;
	
//	printk("EnableSTA:0x%x\n",tmpValue);		
	//tmpValue = BecaonInterval | (DtimPeriod<<16) ;    
	pObj->SetReg(reg, ZD_BCNInterval, 100);
	zd1205_mode = INFRASTRUCTURE_BSS; 
}	

void HW_StopBeacon(void) 
{
	zd_80211Obj_t *pObj=pdot11Obj;
	void *reg = pdot11Obj->reg;
	pObj->SetReg(reg, ZD_BCNInterval, 0);
}	

//ivan
void HW_MakeBeacon(U8 *pBeacon, U16 index)
{
...
//ivan
//printk("HW_SwitchChannel:%d\n",channel);

	switch(pObj->rfMode)
	{
		default:
			FPRINT_V("Invalid RF module parameter 0x%x", (int)pObj->rfMode);
			break;
...
//ivan added
void zd_setBasicRate(unsigned char max_rate)
{
    U32             tmpValue;
    void            *reg = pdot11Obj->reg;

//printk("zd_setBasicRate:0x%x\n",max_rate);


	tmpValue = pdot11Obj->GetReg(reg, ZD_CtlReg1);
//printk("tmpValue=0x%x\n",tmpValue);
	switch(max_rate&0x7f){
		case SR_1M:
			tmpValue &= ~0x1c;
			tmpValue |= 0x00;
			break;
...
//printk("tmpValue2=0x%x\n",tmpValue);	
	pdot11Obj->SetReg(reg, ZD_CtlReg1, tmpValue);
}
...
//printk("HW_InitHMAC\n");	
	...
	pObj->SetReg(reg, ZD_Rx_Filter, 0); 
#else
...
printk("HW_OverWritePhyRegFromE2P\n");
...
void HW_UpdateIntegrationValue(zd_80211Obj_t *pObj, U32 ChannelNo)
{
	U32	tmpvalue;
	void *reg = pObj->reg;
printk("HW_UpdateIntegrationValue\n");
	tmpvalue = pObj->GetReg(reg, ZD_E2P_POD);
	if (tmpvalue & BIT_7){
