...
)
{
	...
	u16			FrameType = wla_hdr->frame_ctrl[0];
	
//printk("setting_parms->rate=%x\n",setting_parms->rate);	
...
//printk("cfg_ctrl_setting len=0x%x\n",len);
					break;
			    case TKIP:			        
					len = curr_frag_len + 40; 	/* Header(24) + CRC32(4) + IV(4) + ExtIV + ICV(4) */
					next_len = next_frag_len + 40;
//printk("cfg_ctrl_setting len=0x%x\n",len);
			        break;
				default:
...
if(PLCP_len)
{
	ctrl_set->ctrl_setting[20] = (u8)PLCP_len;
	ctrl_set->ctrl_setting[21] = (u8)(PLCP_len >> 8);
}
else
{	
...
//printk("hdr_len=%d\n",hdr_len);
//printk("<0:%d>",macp->card_setting.SwCipher);
    if (!macp->card_setting.SwCipher && !send_debug)
    { //hardware encryption
//printk("<1:%x>",pHdr->mac_header[1]);    
		if (pHdr->mac_header[1] & WEP_BIT)
		{ //WEP bit is set
			/* Set IV only for Encryption mode */
//printk("<2:%x>",macp->card_setting.EncryMode);
			switch(macp->card_setting.EncryMode)
			{
				...
				case TKIP: //added by ivan 
		        	pHdr->mac_header[24] = Hi8(macp->iv_16);
		       		pHdr->mac_header[25] = (Hi8(macp->iv_16) | 0x20) & 0x7F;
		        	pHdr->mac_header[26] = Lo8(macp->iv_16);

#if 0
		        	if((pHdr->mac_header[16])&0x1) //group key
		                pHdr->mac_header[27] = (macp->card_setting.EncryKeyId << 6) | 0x20; //with ExtIV
		            else
#endif
		                pHdr->mac_header[27] = 0x20;
		            hdr_len += IV_SIZE;
		        	pHdr->mac_header[28] = (u8)(macp->iv_32 >> 24);
		        	pHdr->mac_header[29] = (u8)(macp->iv_32 >> 16);
		        	pHdr->mac_header[30] = (u8)(macp->iv_32 >> 8);
		        	pHdr->mac_header[31] = (u8)(macp->iv_32);
                    hdr_len += EXTEND_IV_LEN;
                    if(macp->iv_16==0xffff)
                        macp->iv_32++;
                    macp->iv_16++;                    
				    break;
			}
