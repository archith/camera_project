...
#if CONFIG_TS_CPE_MICROWIRE
#define PwDn_InEn_DFR12		0x90
#define X_DFR12				0xD0 /*FPGA org 0x92*/
#define Y_DFR12				0x90 /*FPGA org 0xD2*/
#define ERROR				0x25
#elif CONFIG_TS_CPE_SPI
#define X_DFR12				0xD00000
#define Y_DFR12				0x900000
#define ERROR				0x2
#else
#error "no frame format support"
#endif
...
void DFR_high_resolution(cpe_ts_t *ts,int *x, int *y)
{
    ...
    *(volatile unsigned int *)(ts->io_base + SSP_DATA)=X_DFR12;
...
#elif CONFIG_TS_CPE_SPI
	*x = ((*(volatile unsigned int *)(ts->io_base+SSP_DATA)) >> 3) & 0xFFF;
#endif

    *(volatile unsigned int *)(ts->io_base+SSP_CONTROL2)|=(SSP_TXFCLR|SSP_RXFCLR);
    *(volatile unsigned int *)(ts->io_base + SSP_DATA)=Y_DFR12;
...
#elif CONFIG_TS_CPE_SPI
	*y = 0xFFF - (((*(volatile unsigned int *)(ts->io_base+SSP_DATA)) >> 3) & 0xFFF);
#endif
...
cpets_open(struct inode * inode, struct file * filp)
{
	...
	fLib_SSPClearTxFIFO(CPE_SSP1_VA_BASE);
	fLib_SSPClearRxFIFO(CPE_SSP1_VA_BASE);
	...
	ts->chug_tq.data = ts;
	ts->cur_point.x = TOUCH_PANEL_RIGHT_X;
	ts->cur_point.y = TOUCH_PANEL_RIGHT_Y;
#endif /* end_of_not */
...
//	int i;
	...
	// if PCLK = 44MHz, it's about 3 CLK = 3 us
	// min ADC acquisition time = 1.5 us (3 clk)
...
#if CONFIG_TS_CPE_MICROWIRE
	info("Faraday SSP Touch Panel Registered (Microwire)");
#elif CONFIG_TS_CPE_SPI
	info("Faraday SSP Touch Panel Registered (SPI)");
#endif
...
#ifdef CONFIG_FIE8100_PLATFORM
	cpe_int_set_irq(ts->irq,LEVEL, H_ACTIVE);
#else
