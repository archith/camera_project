...
/* add by Charles Tsai */
#ifdef CONFIG_FARADAY_KMI
extern FaradayLEDS fa_led_state;
void fa_leds_proc(unsigned char scancode, int down);
#endif
/* end add */
...
static void kbd_bh(unsigned long dummy)
{
	...
	if (leds != ledstate) {
		ledstate = leds;		
		//kbd_leds(leds);//mark by Charles Tsai
		if (kbd_ledfunc) kbd_ledfunc(leds);
	}
	
	/* add by Charles Tsai */
#ifdef CONFIG_FARADAY_KMI	
	if (fa_led_state.update){
		kbd_leds();
	}
#endif
	/* end add */
...
void fa_leds_proc(unsigned char scancode, int down)
{
	...
	if (down == 0 && scancode != FA_KMI_ACK){
		fa_led_state.LEDSState[0] = 0;//up
		fa_led_state.LEDSState[1] = 0;//up
		fa_led_state.LEDSState[2] = 0;//up
	}
	if ((scancode == FA_SCROLL_LOCK) && down && fa_led_state.LEDSState[0] == 0){
		fa_led_state.LEDSState[0] = 1;//down
		fa_led_state.update=1;// satet already chage
		if (fa_led_state.LEDSOnOff[0] == 1){//toggle
			fa_led_state.LEDSOnOff[0]=0;
			fa_led_state.LEDS &= ~(0x1);
		}
		else{
			fa_led_state.LEDSOnOff[0]=1;
			fa_led_state.LEDS |= 0x1;
		}
	}

	if ((scancode == FA_NUM_LOCK) && down && fa_led_state.LEDSState[1] == 0){
		fa_led_state.LEDSState[1] = 1;//down
		fa_led_state.update=1;// satet already chage
		if (fa_led_state.LEDSOnOff[1] == 1){//toggle
			fa_led_state.LEDSOnOff[1]=0;
			fa_led_state.LEDS &= ~(0x2);
		}
		else{
			fa_led_state.LEDSOnOff[1]=1;
			fa_led_state.LEDS |= 0x2;
		}
	}

	if ((scancode == FA_CAP_LOCK) && down && fa_led_state.LEDSState[2] == 0){
		fa_led_state.LEDSState[2] = 1;//down
		fa_led_state.update=1;// satet already chage
		if (fa_led_state.LEDSOnOff[2] == 1){//toggle
			fa_led_state.LEDSOnOff[2]=0;
			fa_led_state.LEDS &= ~(0x4);
		}
		else{
			fa_led_state.LEDSOnOff[2]=1;
			fa_led_state.LEDS |= 0x4;
		}
	}
