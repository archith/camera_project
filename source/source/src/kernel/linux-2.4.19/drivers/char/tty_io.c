...
extern void cpe_console_init(void);
...
#ifdef CONFIG_REDIRECT_TTY_TO_CONSOLE
#ifdef CONFIG_VT
    {   extern struct tty_driver console_driver;
        char buf[64];
        if (strcmp(tty_name(tty, buf), "tts/0") == 0 ||
          strcmp(tty_name(tty, buf), "console") == 0) {
            console_driver.table[fg_console] = tty;
        }
    }
#else
...
#if defined(CONFIG_SERIAL) && !defined(CONFIG_ARCH_PL1029)
        rs_init();
#endif
