...
//ivan
//static int debugmem = 1;	/* switched off by default */
...
static int sound_alloc_dmap(struct dma_buffparms *dmap)
{
	...
	start_addr = NULL;
//ivan
//printk("dma_buffersize:0x%x dma_pagesize:0x%x\n",dma_buffsize,dma_pagesize);
	...
	if (start_addr == NULL) {
		...
		//ivan
		if(debugmem)
			printk("dma_pagesize 0x%x, max_dma_address 0x%x\n",dma_pagesize,MAX_DMA_ADDRESS);
...
static int output_sleep(int dev, int dontblock)
{
	...
	if (timeout != MAX_SCHEDULE_TIMEOUT && !timeout_value) {
		...
		//printk("dev:%d\n", dev);/* add by Charles Tsai */
		 dma_reset_output(dev); // mark by Charles Tsai
	} else {
		if (signal_pending(current))
			err = -EINTR;
	}
...
static void finish_output_interrupt(int dev, struct dma_buffparms *dmap)
{
	...
	/*add by Charles Tsai */
	//printk("wake up finish output:%d \n", dev);
}
...
static void do_outputintr(int dev, int dummy)
{
	...
	while (dmap->qlen <= -dmap->closing) {
		printk("OSS sound play underrun!\n"); //ivan wang for underrun notifing
		dmap->underrun_count++;
		dmap->qlen++;
