...
static int __init fi2s_init(void)
{
    ...
    fi2s_cfg.irq= 0; //john IRQ_A321_I2S;
    ...
    strcpy(dev_info->name,"A321 APB DMA");
    cpe_int_set_irq(IRQ_A321_APB_DMA, LEVEL, H_ACTIVE);
    if(request_irq(IRQ_A321_APB_DMA,dma_interrupt_handler,SA_INTERRUPT,"A321 APB DMA",(void *)my_dev)<0)
    {
