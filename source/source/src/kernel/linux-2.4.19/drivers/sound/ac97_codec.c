...
static void ac97_write_mixer(struct ac97_codec *codec, int oss_channel, int mute,
...
static const struct {
	...
	{0x414C4710, "Realtek ALC201/201A",	&null_ops},
    {0x414c4740, "Realtek ALC202/202A", &null_ops},
    {0x414c4770, "Realtek ALC203/203LF", &null_ops},
...
#else /* MODIFY for Prolific Audio Controlling */
		      ...
		      unsigned int left, unsigned int right)
{
	...
	if (AC97_STEREO_MASK & (1 << oss_channel))
	{
		...
		else
		{
			...
			right = (((100 - right) * scale)+50) / 100;
			left = (((100 - left) * scale)+50) / 100;
			if (right >= scale)
				right = scale-1;
			if (left >= scale)
				left = scale-1;
...
static void ac97_set_mixer(struct ac97_codec *codec, unsigned int oss_mixer, unsigned int val )
{
	...
	codec->mixer_state[oss_mixer] = mute | (right << 8) | left;
	codec->write_mixer(codec, oss_mixer, mute, left, right);
}
#endif
...
static int ac97_mixer_ioctl(struct ac97_codec *codec, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == SOUND_MIXER_INFO) {
        ...
        strncpy(info.id, "AC97", sizeof(info.id));
    ...
    /* direct access ac97 register */
    if (_IOC_TYPE(cmd) == 'A' && _SIOC_SIZE(cmd) == sizeof(int))
    {
        int reg;
        reg = _IOC_NR(cmd);
        if (reg & 0x1)      /* register must even address */
            return -EINVAL;
    	if (_SIOC_DIR(cmd) == _SIOC_READ) {
            val = codec->codec_read(codec, reg);
...
static int ac97_init_mixer(struct ac97_codec *codec)
{
	...
	i  = codec->codec_read(codec, AC97_HEADPHONE_VOL);
	printk("read AC97_HEADPHONE_VOL: 0x%04x\n", i);

	if(i != 0x1515)
	{
		printk("AC97_HEADPHONE_VOL is not the initial value, reset it.\n");
		udelay(1000);
		codec->codec_write(codec, AC97_HEADPHONE_VOL, 0x1515);
		udelay(1000);
		i  = codec->codec_read(codec, AC97_HEADPHONE_VOL);
		printk("After resetting, AC97_HEADPHONE_VOL: 0x%04x\n", i);
	}

	udelay(10000);
	i  = codec->codec_read(codec, AC97_MIC_VOL);
	printk("read AC97_MIC_VOL: 0x%04x\n", i);

	if(i != 0x801f)
	{
		printk("AC97_MIC_VOL is not the initial value, reset it.\n");
		udelay(10000);
		codec->codec_write(codec, AC97_MIC_VOL, 0x801f);
		udelay(10000);
		i = codec->codec_read(codec, AC97_MIC_VOL);
		printk("After resetting, AC97_MIC_VOL: 0x%04x\n", i);
	}

	printk("End to initialize the mixer.\n");
...
{
	int i;
	unsigned int left, right, val, mute;
	...
	for (i = 0; i < SOUND_MIXER_NRDEVICES; i++) {
		...
		left = val  & 0xff;
        mute = val & 0x8000;
		codec->write_mixer(codec, i, mute, left, right);
	}
	return 0;
}
#endif
