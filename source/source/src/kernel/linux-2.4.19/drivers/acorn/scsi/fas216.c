...
#ifdef DEBUG_CONNECT
	printk("scsi%d.X: Connected: %1X %1X %02X, reconnected: %1X %1X %02X\n",
		info->host->host_no,
...
#ifdef DEBUG_CONNECT
	printk("scsi%d.%c: data pointers [%p, %X]\n",
		info->host->host_no, '0' + SCpnt->target,
		info->scsi.SCp.ptr, info->scsi.SCp.this_residual);
#endif
...
fas216_std_done(FAS216_Info *info, Scsi_Cmnd *SCpnt, unsigned int result)
{
...
#ifdef DEBUG_CONNECT
	printk("scsi%d.%c: command complete result=0x%08x CDB: ",
		info->host->host_no, '0' + SCpnt->target, SCpnt->result);
	print_command(SCpnt->cmnd);
#endif

	/*
	 * If there are any error conditions in the driver, we
	...
	default:
		printk(KERN_ERR "scsi%d.%c: incomplete data transfer "
			...
			info->host->host_no, '0' + SCpnt->target);
		print_command(SCpnt->cmnd);
		printk(KERN_ERR " result 0x%08x ptr %p len 0x%x "
			"totlen 0x%x rq_totlen 0x%x\n",
...
request_sense:
	...
	SCpnt->SCp.Message = 0;
	SCpnt->SCp.Status = 0;
	SCpnt->request_buffer = (char *)SCpnt->sense_buffer;
...
#if 0
		printk(KERN_WARNING "scsi%d.%c: zero length buffer "
		       "passed for command ", info->host->host_no,
		       '0' + SCpnt->target);
		print_command(SCpnt->cmnd);
#endif
		SCpnt->SCp.ptr = NULL;
