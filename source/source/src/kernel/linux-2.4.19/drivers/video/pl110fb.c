...
static struct pl110fb_rgb def_rgb_16 = {
	transp:	{ offset: 16,  length: 1, msb_right: 0},
...
static struct pl110fb_mach_info Sharp_LQ057Q3DC02_info __initdata =
{
	...
	time2:
		...
		|(LCD_FALSE	<< 14) 			//invert output enable
		|(LCD_FALSE	<< 13)  //john|(LCD_TRUE	<< 13) 			//invert panel clock
		|(LCD_TRUE	<< 12) 			//invert horizontal sync.
		|(LCD_TRUE	<< 11) 			//invert vertical sync
		|((32 - 1)	<<  6) 			//ac bias
		|(LCD_FALSE	<<  5) 			//clock select
...
static struct pl110fb_mach_info AUO_A036QN01_info[3] __initdata =
{
	{   //AMBA = 33 MHZ
    time0:    0x2B20144C,
	time1:    0x0B0808EF,
...
static struct pl110fb_mach_info * __init pl110fb_get_machine_info(struct pl110fb_info *fbi)
{
	 ...
	 */
	///inf = &Sharp_LQ084C1DG21_info;   //sharp panel
//	inf = &Sharp_LQ057Q3DC02_info;

if (CONFIG_SYS_CLK<=45000000)
	inf = &AUO_A036QN01_info[0];   //AUO panel
else if (CONFIG_SYS_CLK<=65000000)  
	inf = &AUO_A036QN01_info[1];   //AUO panel
else 
	inf = &AUO_A036QN01_info[2];   //AUO panel
...
int __init pl110fb_init(void)
{
	...
	printk("pl110fb_init, test message....\n");

	// John, let LCD have more drive power
	pmu_val = *(unsigned int *)(CPE_PMU_VA_BASE+0x44);
	pmu_val = (pmu_val & 0xff0fffff) | 0x00c00000;
	*(unsigned int *)(CPE_PMU_VA_BASE+0x44)=pmu_val;
