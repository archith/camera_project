...
static struct serial_uart_config uart_config[] = {
    ...
    { "PL UART", 2, UART_CLEAR_FIFO | UART_USE_FIFO },
	{ 0, 0}
};
...
#define CON_RX_STATUS   10 /* read status & FIFO data at once */
#endif
...
unsigned int pl_con(int op, struct async_struct *info, int data)
{

    switch(op) {
    case CON_STATUS:
	return byteAddr(rCONSOLE_STATUS);
    case CON_RXRDY:
        return (byteAddr(rCONSOLE_STATUS) & PL_UART_LSR_DR);
    case CON_RX:
        return byteAddr(rCONSOLE_DATA);
    case CON_TXRDY:
        return ((byteAddr(rCONSOLE_STATUS) & PL_UART_LSR_TFUL) == 0);
    case CON_TX:
        byteAddr(rCONSOLE_DATA) = (unsigned char) data;
        break;
    case CON_BREAK:
        byteAddr(rCONSOLE_SRB) = 0;
        break;
    case CON_DETECT:
        return ((byteAddr(rCONSOLE_BAUD_ST) & 0x81) == 0x80);
#if defined(CONFIG_ARCH_PL1071) ||  defined(CONFIG_ARCH_PL1091)
    case CON_RX_STATUS:
	return halfAddr(rCONSOLE_STATUS); /* read both status & FIFO data */
#endif
    }
...
#if defined(CONFIG_ARCH_PL1071) ||  defined(CONFIG_ARCH_PL1091)
		ch = (unsigned char)(((*status)& 0xff00) >> 8);
                *status = (*status)& 0xff;
#else
...
#endif
#if defined(CONFIG_ARCH_PL1071) ||  defined(CONFIG_ARCH_PL1091)
		*status = pl_con(CON_RX_STATUS, info, 0);
#else
...
void __init pl_console_init(void)
{
