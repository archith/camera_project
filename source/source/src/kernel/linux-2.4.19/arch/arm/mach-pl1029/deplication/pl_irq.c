/*
 *  linux/arch/arm/mach-pl1091/pl1091-irq.c ,copy from ..
 ...
 *
 *  Modified by Jim Lee(2005/03/25)
 *  The main purpose is to modifiy get_irq_list function.
 *
 */
...
char *
status2bit(unsigned long val)
{
    int i, j;
    char *histr = "3210__R__BTPCZSI76543210__KIKIKI";
    char *lostr = "....__r__btpczsi........__kikiki";
    static char bitstr[48];

    for (i = 0, j = 0; i < 32; i++) {
        bitstr[j++] = (val & (1L << (31 - i))) ? histr[i] : lostr[i];
        if (((i+1) % 8) == 0)
            bitstr[j++] = ' ';
    }
    bitstr[j]= '\0';
...
char *mask2bit(unsigned short val, char hi)
{
...
int get_irq_list(char *buf)
{
  ...
  unsigned short intc_p, dmac_p;
    ...
    intc_s = readw(rINTERRUPT_MASK0);
    dmac_s = readw(rDMAC0_INT_MASK);

    intc_p = readw(rINTERRUPT_STATUS0);
    dmac_p = readw(rDMAC0_INT_STATUS);
...
void do_IRQ(int irq, struct pt_regs * regs)
{
	 ...
	 */
	desc->mask_ack(irq);  // by Jim Lee(2005/03/25)
