...
static int do_fa526_fault(unsigned long addr, int error_code, struct pt_regs *regs)
{
    printk("do_fa526_fault:addr=0x%lx,error_code:0x%x\n",addr,error_code);
    if( (addr>=PAGE_OFFSET) && (addr<=PAGE_OFFSET+DRAM_SIZE) )
        return do_sect_fault(addr,error_code,regs);
    else
...
static const struct fsr_info {
	...
	{ do_fa526_fault,		SIGSEGV, "FA526 exception"	   },
...
//debug by ivan
char abort_type=0; //0:data abort 1:prefetch abort
...
#if 0
if(fsr==13)
	printk("section permission abort:addr=0x%x\n",addr);
else if(fsr==15)
	printk("page permission abort:addr=0x%x\n",addr);
#endif
