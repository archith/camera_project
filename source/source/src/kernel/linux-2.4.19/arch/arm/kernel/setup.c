...
{
	unsigned pmode;
	pmode=*(volatile unsigned int *)(CPE_PMU_VA_BASE+0xc);
	pmode >>= 4;
...
static unsigned int u32ReadHCLK(void)
{
    ...
    mul = ((*(unsigned int *)(CPE_PMU_VA_BASE+0x30))>>3)&0x1ff;
    hclk=(36864U*mul+1)*100;
    return hclk;
}
int cpe_get_ahb_clk(void)
{
	static unsigned ahb_clk=0;
	if(!ahb_clk) {
		ahb_clk=u32ReadHCLK()/getDivFactor();
		printk("A320: Autodetected AHB Clock:");
		printk(" CPU/AHB=%dHz/%dHz\n", hasTurbo()?u32ReadHCLK():ahb_clk, ahb_clk);
	}
	return ahb_clk;
}
...
static char command_line[COMMAND_LINE_SIZE];	/* should no use */
...
static void __init setup_processor(void)
{
...
#if defined(CONFIG_CPU_FA520) || defined(CONFIG_CPU_FA526)

	{
		/* read cache size */
		unsigned char *arm_cache_size[] =
		 	{"512B","1KB","2KB","4KB","8KB","16KB","32KB","64KB"};
		unsigned int cr1, tmp;
		__asm__ volatile ("mrc p15,0,%0,c0,c0,1":"=r"(cr1));
		tmp=(cr1&0x1c0)>>6;
		...
		if(tmp<8)
			...
			printk("ICache:%s enabled, ",arm_cache_size[tmp]);
#else
			printk("ICache:%s disabled, ",arm_cache_size[tmp]);
#endif
		tmp=(cr1&0x1c0000)>>18;
		if(tmp<8)
#ifndef CONFIG_CPU_DCACHE_DISABLE
			printk("DCache:%s enabled",arm_cache_size[tmp]);
#else
			printk("DCache:%s disabled",arm_cache_size[tmp]);
#endif
    ...
    sprintf(system_utsname.machine, "%s%c (A320D)", list->arch_name, ENDIANNESS);
#endif
#else
...
/*
 * FIX: Original method will over-written the default command line (from
 *      .config. It always use "tag" input (from loader).
 *      Now I append the loader tags after the default command line option.
 */
static int __init parse_tag_cmdline(const struct tag *tag)
{
	int len1, len2;
	char tmp_cmdline1[COMMAND_LINE_SIZE];
	char tmp_cmdline2[COMMAND_LINE_SIZE];

	/* -- Cmdline: TAG -- */
	memset(&tmp_cmdline1[0], '\0', COMMAND_LINE_SIZE);
	strncpy(tmp_cmdline1, tag->u.cmdline.cmdline, COMMAND_LINE_SIZE);
	tmp_cmdline1[COMMAND_LINE_SIZE - 1] = '\0';
	len2 = strlen(tmp_cmdline1);
	/* -- CmdLine: .config -- */
	memset(&tmp_cmdline2[0], '\0', COMMAND_LINE_SIZE);
	strncpy(tmp_cmdline2, default_command_line,   COMMAND_LINE_SIZE);
	tmp_cmdline2[COMMAND_LINE_SIZE - 1] = '\0';
	/* Do NOT over-written, "append" on the tail!! */
	if (len2 != 0) {
		len1 = strlen(tmp_cmdline2);
		if ((len1 + len2) >= (COMMAND_LINE_SIZE - 1)) {
			tmp_cmdline1[COMMAND_LINE_SIZE - len1 - 2] = '\0'; /* rest size for TAG CmdLine */
		} /* need to consider the <SPACE> between 2 cmdlines */
		sprintf(default_command_line, "%s %s", tmp_cmdline2, tmp_cmdline1);
	}
