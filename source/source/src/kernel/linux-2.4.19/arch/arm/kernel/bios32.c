...
#ifdef CONFIG_ARCH_CPE  //modified by ivan
//    printk("res->flags=0x%x val=0x%x phys=0x%x\n",res->flags,val,PHY_ADDRESS(val));
...
extern struct hw_pci cpe_pci;	//paulong 20030623
...
#endif
#ifdef CONFIG_ARCH_CPE	//Robert 20030620
		if (machine_is_cpe()) {
			hw = &cpe_pci;
			break;
		}
