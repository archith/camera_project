...
#if defined(CONFIG_ARCH_PL1097) || defined(CONFIG_ARCH_PL1029)
    {
    extern int get_arch_irq_list(char *p);
        p += get_arch_irq_list(p);
    }
#endif
...
{
	struct irqdesc *desc;
	irq = fixup_irq(irq);       /* jedy add to fixup lvl3 irq */
