...
#if 0
asmlinkage void do_FIQ(struct pt_regs * regs)			// irq ¡ 32~64
{
	struct irqdesc * desc;
	...
	regs = NULL;				// waiting to do:
	for (;;)
	{
		irq_source_bits = inl(CPE_IC_BASE+FIQ_STATUS_REG);
		if (irq_source_bits==0)
		{
			break;
		}

		for (irq=0; (irq_source_bits & 0x1)==0; ++irq)
		{
			irq_source_bits >>= 1;
		}
		irq += CPE_NR_IRQS;
