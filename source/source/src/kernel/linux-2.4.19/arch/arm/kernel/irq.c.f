...
///ivan
#if 0
	if (irq==IRQ_EXT_A321 ||irq>=CPE_VIRQ_PCI_START)
	    printk("<%d>",irq);
#endif
...
//fiq should not be interruptable (modified by ivan)
		//if (!(action->flags & SA_INTERRUPT))
		if (!(action->flags & SA_INTERRUPT) && (irq<CPE_NR_IRQS))
			local_irq_enable();
...
///ivan
    ...
    //fiq should not be interruptable (modified by ivan)
	//if (softirq_pending(cpu))
	if (softirq_pending(cpu)&&(irq<CPE_NR_IRQS))
		do_softirq();
	return;
...
void free_irq(unsigned int irq, void *dev_id)
{
	...
	if (irq >= NR_IRQS || !irq_desc[irq].valid) {
		printk(KERN_ERR "Trying to free IRQ%d, valid = %d\n",irq, irq_desc[irq].valid);
	...
	printk(KERN_ERR "Trying to free free IRQ%d, dev_id = %08X\n",irq, (u32) dev_id);
#ifdef CONFIG_DEBUG_ERRORS
	__backtrace();
#endif
out:
