...
static __inline__ void *fb_memset255(void *s, size_t count)
{
    ...
    if (xs & 1) {
	//fb_writeb(0xff, xs++);//ivan
	fb_writeb(0xff, (u8 *)xs++);
	count--;
    }
    if (xs & 2) {
	//fb_writew(0xffff, xs);//ivan
	fb_writew(0xffff, (u16 *)xs);
	xs += 2;
	count -= 2;
    }
    while (count > 3) {
	//fb_writel(0xffffffff, xs);//ivan
	fb_writel(0xffffffff, (u32 *)xs);
	xs += 4;
	count -= 4;
    }
rest:
...
static __inline__ void *fb_memmove(void *d, const void *s, size_t count)
{
	...
	if (dst & 1) {
	    //fb_writeb(fb_readb(src++), dst++);//ivan
	    fb_writeb(fb_readb(src++), (u8 *)dst++);
	    count--;
	}
	if (dst & 2) {
	    //fb_writew(fb_readw(src), dst);//ivan
	    fb_writew(fb_readw(src), (u16 *)dst);
	    src += 2;
	    dst += 2;
	    count -= 2;
	}
	while (count > 3) {
	    //fb_writel(fb_readl(src), dst);//ivan
	    fb_writel(fb_readl(src), (u32 *)dst);
	    src += 4;
	    dst += 4;
	    count -= 4;
	}
