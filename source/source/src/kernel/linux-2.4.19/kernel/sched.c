...
repeat_schedule:
	 ...
	 */
//ivan
//if(!strcmp(next->comm,"sh"))
//	__asm__("mov r0,r0");
#if 0 
if(strcmp(prev->comm,next->comm))
	printk("%s->%s(0x%x)\n",prev->comm,next->comm,(unsigned int)next);
if(!strcmp(next->comm,"swapper"))
	__asm__("mov r0,r0");
#endif
 ...
 */
/* jedy:: add __cond_resched and yield for crypto component porting */
void yield(void)
{
