...
static ssize_t generic_file_direct_IO(int rw, struct file * filp, char * buf, size_t count, loff_t offset)
{
	...
	while (count > 0) {
        ...
        // flush cache to memory if write
        // invalidate cache if read
        if (rw==WRITE) 
	       flush_dcache_range((unsigned long)buf, (unsigned long)buf + iosize);
	    else //READ
	       invalidate_dcache_range((unsigned long)buf, (unsigned long)buf+ iosize);

		retval = mapping->a_ops->direct_IO(rw, inode, iobuf, (offset+progress) >> blocksize_bits, blocksize);

		// flush read data to memory
	    pgcount = iobuf->nr_pages;
	    if (rw == READ) {
	       while (pgcount--) {
