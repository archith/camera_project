...
PWD := \033[100D\033[25C [$(subst $(TOPDIR),.,$(CURDIR))]
...
%.o: %.s
	@if [ "$(SILENT)" == "y" ]; then echo -e "--- asm $< $(PWD) ---"; fi
...
ifdef O_TARGET
$(O_TARGET): $(obj-y)
	@if [ "$(SILENT)" == "y" ]; then echo -e "=== target $@ $(PWD) ==="; fi
	rm -f $@
    ifneq "$(strip $(obj-y))" ""
...
ifdef L_TARGET
$(L_TARGET): $(obj-y)
	@if [ "$(SILENT)" == "y" ]; then echo -e "--- library $@ $(PWD) ---"; fi
	rm -f $@
...
ifdef SUB_DIRS
$(subdir-list) : dummy
	@if [ "$(SILENT)" == "y" ]; then echo -e ">>> [$(MAKELEVEL)] make -C $(subst $(TOPDIR),.,$(CURDIR))/$(patsubst _subdir_%,%,$@) >>>"; fi
