    ...
    IN	unsigned long data) 
{
    ...
    DBGPRINT(RT_DEBUG_TRACE,"ASSOC - enqueue MT2_ASSOC_TIMEOUT ,set R17LowerUpperSelect=%d \n",!pAd->BbpTuning.R17LowerUpperSelect);
...
{
    RTMP_ADAPTER *pAd = (RTMP_ADAPTER *)data;
    DBGPRINT(RT_DEBUG_TRACE,"ASSOC - enqueue MT2_REASSOC_TIMEOUT ,set R17LowerUpperSelect=%d \n",!pAd->BbpTuning.R17LowerUpperSelect);
    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
    ...
    {   
		...
		/* carella modify */
		if((pAd->PortCfg.Channel > 14) && (pAd->PortCfg.bIEEE80211H == TRUE))
            ...
            (pAd->PortCfg.AuthMode == Ndis802_11AuthModeWPAPSK))            
		{
            ...
            // Modify Group cipher
			//CipherTmp[11] = ((pAd->PortCfg.GroupCipher == Ndis802_11Encryption2Enabled) ? 0x2 : 0x4); //<--carella modify: Cipher Type in Mix Mode will fail. 
			if(pAd->PortCfg.GroupCipher == Ndis802_11Encryption2Enabled)
				CipherTmp[11] = 0x2;
			else if(pAd->PortCfg.GroupCipher == Ndis802_11Encryption3Enabled)
				CipherTmp[11] = 0x4;
			else
				CipherTmp[11] = 0x1; //-->carella
				 ...
				 (pAd->PortCfg.AuthMode == Ndis802_11AuthModeWPA2PSK))
		{
			...
			// Modify Group cipher
			//CipherTmp[7] = ((pAd->PortCfg.GroupCipher == Ndis802_11Encryption2Enabled) ? 0x2 : 0x4); //<--carella modify: Cipher Type in Mix Mode will fail.
			if(pAd->PortCfg.GroupCipher == Ndis802_11Encryption2Enabled)
				CipherTmp[7] = 0x2;
			else if(pAd->PortCfg.GroupCipher == Ndis802_11Encryption3Enabled)
				CipherTmp[7] = 0x4;
			else
				CipherTmp[7] = 0x1; //-->carella
			// Modify Pairwise cipher
		...
		{
			if ((pAd->PortCfg.bPiggyBackCapable) && ((pAd->MlmeAux.APRalinkIe & 0x00000003) == 3))
			{
				ULONG TmpLen;
	    ...
	    RTMPSetTimer(pAd, &pAd->MlmeAux.AssocTimer, Timeout); /* in mSec */
    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
    ...
    else if(MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv))
    {
	    ...
	    RTMPSetTimer(pAd, &pAd->MlmeAux.ReassocTimer, Timeout);/* in mSec */
    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
    ...
    RTMPSetTimer(pAd, &pAd->MlmeAux.DisassocTimer, Timeout);   /* in mSec */
    ...
    IN PEDCA_PARM pEdcaParm)
{
 ...
 // Suzhou update it, Because some times the AP has been removed from the bssTable, but can connect to it .
  // If not use the AP's auth infos, it will auth fail. May cause the connect timeout issue
#if 0
	else
#endif
	{
