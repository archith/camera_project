...
//	UCHAR			Offset = 0;  // remove by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
	
	DBGPRINT(RT_DEBUG_TRACE, "-----> WpaEAPOLKeyAction\n");

    pHeader = (PHEADER_802_11) Elem->Msg;
    
//	if ((pHeader->FC.SubType & 0x08) && OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED))
//		Offset += 2;	  

	// Get 802.11 header first
	// edit by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
//	pKeyDesc = (PKEY_DESCRIPTER) &Elem->Msg[(LENGTH_802_11 + LENGTH_802_1_H + Offset + LENGTH_EAPOL_H)];
...
//		Offset += 2;	  
	...
	// edit by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
//	pMsg1 = (PEAPOL_PACKET) &Elem->Msg[LENGTH_802_11 + LENGTH_802_1_H + Offset];
	...
	if((OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED))&&(pHeader->FC.SubType==SUBTYPE_QDATA))
...
//		Offset += 2;	  
	...
	// edit by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
//	pMsg3 = (PEAPOL_PACKET) &Elem->Msg[LENGTH_802_11 + LENGTH_802_1_H + Offset];
...
//	UCHAR				Offset = 0;  // remove by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
	...
	mpool = kmalloc(1152, MEM_ALLOC_FLAG);  // allocate memory
	if (mpool == NULL)
		return;
	...
	// Key Len = 32.
	Key = (UCHAR *) ROUND_UP(KEYDATA + 512, 4);
	// DescKeyData Len = 512
	DescKeyData = (UCHAR *) ROUND_UP(Key+512, 4);
...
//	pMsg3 = (PEAPOL_PACKET) &Elem->Msg[LENGTH_802_11 + LENGTH_802_1_H + Offset];
	...
	NdisMoveMemory( DescKeyData, 
					// edit by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
//					&Elem->Msg[LENGTH_802_11 + LENGTH_802_1_H + Offset + sizeof(EAPOL_PACKET) - MAX_LEN_OF_RSNIE], 
...
//		Offset += 2;
	...
	// edit by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
//   	if(NdisEqualMemory(&Elem->Msg[LENGTH_802_11 + LENGTH_802_1_H + Offset], EAPHEAD, LENGTH_802_1_H))
   	if(NdisEqualMemory(&Elem->Msg[LENGTH_802_11 + LENGTH_802_1_H], EAPHEAD, LENGTH_802_1_H))
	// end johnli
   	{
		DBGPRINT(RT_DEBUG_TRACE, "WpaGroupMsg1Action ----->MsgLen=%d\n",Elem->MsgLen);
   	   	NdisMoveMemory(MSG, Elem->Msg, LENGTH_802_11);
		NdisMoveMemory(&MSG[LENGTH_802_11], &Elem->Msg[LENGTH_802_11+ LENGTH_802_1_H], (Elem->MsgLen ));
   	}
   	else
   	{
	...
	// edit by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
//	pGroup = (PEAPOL_PACKET) &MSG[LENGTH_802_11 + LENGTH_802_1_H + Offset];
	...
	NdisMoveMemory( DescKeyData, 
			// edit by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
//			&MSG[LENGTH_802_11 + LENGTH_802_1_H + Offset + sizeof(EAPOL_PACKET) - MAX_LEN_OF_RSNIE], 
