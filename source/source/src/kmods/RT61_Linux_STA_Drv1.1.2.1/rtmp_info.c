...
static struct {
	...
	{"BlockChannelBitMap",				Set_BlockChannelBitMap_Proc}, 
	{"BGProtection",				Set_BGProtection_Proc},
	{"TxPreamble",					Set_TxPreamble_Proc},         
	{"RTSThreshold",				Set_RTSThreshold_Proc},       
	{"FragThreshold",				Set_FragThreshold_Proc},      
	{"TxBurst",						Set_TxBurst_Proc},          
	{"ShowTx",			Set_ShowTx_Proc},
...
//	{"ShortSlot",					Set_ShortSlot_Proc},
	...
	{"ScanTabClean",						Set_ScanTabClean_Proc},  
	{"R17D",						Set_R17D_Proc},  
	{"ShowScanTab",						Set_ShowScanTab_Proc},  
#endif          
#ifdef RALINK_ATE   
		   ...
		   struct iw_freq *freq, char *extra)
{
	PRTMP_ADAPTER pAdapter = (PRTMP_ADAPTER) dev->priv;
	ULONG	m;
	DBGPRINT(RT_DEBUG_INFO,"==>rt_ioctl_giwfreq  %d\n",pAdapter->PortCfg.Channel);
		   ...
		   __u32 *mode, char *extra)
{
	...
	if (pAdapter->PortCfg.bNativeWpa == FALSE) //<--carella modify
		return -ENETDOWN; //-->carella
		   ...
		   __u32 *mode, char *extra)
{
	...
	DBGPRINT(RT_DEBUG_INFO,"==>rt_ioctl_giwmode\n");		
	return 0;
}
		   ...
		   struct iw_point *data, char *extra)
{
	...
	range->event_capa[0] = (IW_EVENT_CAPA_K_0 | IW_EVENT_CAPA_MASK(SIOCGIWAP) | IW_EVENT_CAPA_MASK(SIOCGIWSCAN));
					...
					union iwreq_data *wrqu, char *extra)
{
	...
	{
		OPSTATUS_CLEAR_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED); //carella add
	...
	do{
		Now = jiffies;
            ...
            if (pAdapter->Mlme.CntlMachine.CurrState != CNTL_IDLE)
            {
                ...
                DBGPRINT(RT_DEBUG_TRACE, "!!! MLME busy, reset MLME state machine ,%d!!!\n",pAdapter->Mlme.CntlMachine.CurrState);
            }
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	PRTMP_ADAPTER pAdapter = (PRTMP_ADAPTER) dev->priv;
	int i=2, j;
	char *current_val, *current_ev = extra, *previous_ev = extra;
	struct iw_event iwe;
	BSS_ENTRY *pBssEntry;
#if WIRELESS_EXT >= 17
...
//#endif // NATIVE_WPA_SUPPLICANT_SUPPORT //
	...
	for (i = 0; i < pAdapter->ScanTab.BssNr; i++) 
	{
		...
		if (pBssEntry->VarIELen > 0)
		{
			EID_STRUCT *pEid;
			UCHAR *pEnd;
			int EIDLen, isWPA, isRSN, isWPAUpdate, isRSNUpdate;
			char essid[32];
			int varie_cnt = 0;
			unsigned char *pTmp, *pFromVarIE, *pToVarIE;
			...
			while ( (((UCHAR *)pEid) + sizeof(EID_STRUCT)) <= pEnd)
			{
				...
				isWPA = isRSN = isWPAUpdate = isRSNUpdate = 0;
				switch(pEid->Eid)
				{
					case IE_WPA:
				...
				if(isWPAUpdate || isRSNUpdate)
				{
					pToVarIE = pEid;
					pFromVarIE = pToVarIE + EIDLen;
					pEid = (PEID_STRUCT *)pFromVarIE;
				}
				else
				{
					pFromVarIE = (unsigned char *)pEid; //<--carella modify: WPA + RSN MixMode
					pFromVarIE = pFromVarIE + (2 + pEid->Len);
					pEid = (PEID_STRUCT *)pFromVarIE;
					//pEid += (2 + pEid->Len); //-->carella
				}
			}
		}
...
// For wireless_extension >= 17, now can send error back to request more buffer!
	...
	DBGPRINT(RT_DEBUG_TRACE,"<---- %s(): %d(%d) BSS returned\n", __FUNCTION__, i, pAdapter->ScanTab.BssNr);
	return 0;
}
#else
int
			 ...
			 struct iw_point *data, char *essid)
{
	...
	if (pAdapter->PortCfg.bNativeWpa == TRUE)  // add by johnli
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, "Enqueue remove all AsicShareKey Table cmd first!\n");
#if 0
	    ...
	    (pAdapter->PortCfg.IEEE8021X != TRUE))
	{
		...
		for (KeyIdx = 0; KeyIdx < 4; KeyIdx++)
		{
			...
			if(pAdapter->PortCfg.DesireSharedKey[KeyIdx].KeyLen != 0)
			{
				pAdapter->SharedKey[KeyIdx].KeyLen = pAdapter->PortCfg.DesireSharedKey[KeyIdx].KeyLen;
				NdisMoveMemory(pAdapter->SharedKey[KeyIdx].Key, pAdapter->PortCfg.DesireSharedKey[KeyIdx].Key, pAdapter->SharedKey[KeyIdx].KeyLen);

				pAdapter->SharedKey[KeyIdx].CipherAlg = pAdapter->PortCfg.DesireSharedKey[KeyIdx].CipherAlg;

				AsicAddSharedKeyEntry(pAdapter, 0, (UCHAR)KeyIdx, pAdapter->SharedKey[KeyIdx].CipherAlg, pAdapter->SharedKey[KeyIdx].Key, NULL, NULL);
			}
			else
			{
				UCHAR CipherAlg;

				if (WepKey.keyinfo.KeyLength == 5)
			  ...
			  struct iw_point *erq, char *keybuf)
{
	...
	if ((erq->flags & IW_ENCODE_DISABLED) == 0) 
	{
		...
		else
		{
			...
			if ((len == WEP_SMALL_KEY_LEN) || (len == WEP_LARGE_KEY_LEN))
			{
			...
			for (i=0; i < 5;i++)
				DBGPRINT(RT_DEBUG_TRACE,"   =%x ", pAdapter->SharedKey[pAdapter->PortCfg.DefaultKeyId].Key[i]);
			DBGPRINT(RT_DEBUG_TRACE,   "\n");

                // set key into ASIC
		        pAdapter->SharedKey[pAdapter->PortCfg.DefaultKeyId].CipherAlg =
		            (pAdapter->SharedKey[pAdapter->PortCfg.DefaultKeyId].KeyLen == WEP_SMALL_KEY_LEN) ? CIPHER_WEP64 : CIPHER_WEP128;						

		        AsicAddSharedKeyEntry(pAdapter, 
		                                0, 
		                                (UCHAR)pAdapter->PortCfg.DefaultKeyId, 
		                                pAdapter->SharedKey[pAdapter->PortCfg.DefaultKeyId].CipherAlg, 
  //                                      pAdapter->SharedKey[pAdapter->PortCfg.DefaultKeyId].Key,
	...
	// For NATIVE_WPA_SUPPLICANT_SUPPORT support.
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) dev->priv;
	union {
		char buf[sizeof(NDIS_802_11_WEP)+MAX_LEN_OF_KEY- 1];
		NDIS_802_11_WEP keyinfo;
...
#ifdef NATIVE_WPA_SUPPLICANT_SUPPORT
	if (pAd->PortCfg.bNativeWpa == FALSE) //<--carella modify
		return -ENETDOWN; //-->carella
#endif // NATIVE_WPA_SUPPLICANT_SUPPORT //
	...
	if ((erq->length > IW_ENCODING_TOKEN_MAX) 
		...
		DBGPRINT(RT_DEBUG_TRACE, "%s(): index - %d, invalid!\n", __FUNCTION__, index);
		return -EINVAL;
	}
	...
	if (erq->flags & IW_ENCODE_RESTRICTED)	{
		DBGPRINT(RT_DEBUG_TRACE, "%s(): Ndis802_11AuthModeShared\n", __FUNCTION__);
		pAd->PortCfg.AuthMode = Ndis802_11AuthModeShared;
	}
	else {
		DBGPRINT(RT_DEBUG_TRACE, "%s(): Ndis802_11AuthModeOpen\n", __FUNCTION__);
		pAd->PortCfg.AuthMode = Ndis802_11AuthModeOpen;
	}
			  ...
			  struct iw_point *erq, char *key)
{
	...
	DBGPRINT(RT_DEBUG_INFO, "===>rt_ioctl_giwencode %d\n", erq->flags & IW_ENCODE_INDEX);
...
#ifdef NATIVE_WPA_SUPPLICANT_SUPPORT
	DBGPRINT(RT_DEBUG_TRACE, "----> %s(): param.flags[%04x] IW_AUTH_INDEX[%04x]\n", __FUNCTION__, param->flags, IW_AUTH_INDEX);
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	if((keyIdx < 0) || (keyIdx > NR_WEP_KEYS)) {
		DBGPRINT(RT_DEBUG_TRACE,"%s(): keyIdx is invalid - %d, flags - %d\n", __FUNCTION__, keyIdx, encoding->flags);
		return -EINVAL;
	}
	else
		DBGPRINT(RT_DEBUG_TRACE,"%s(): keyIdx - %d\n", __FUNCTION__, keyIdx);
	...
	if ((encoding->flags & IW_ENCODE_DISABLED) || (ext->alg && IW_ENCODE_ALG_NONE))
	{
		...
		RTMPWPARemoveKeyProc(pAd, (PVOID)&RtmpRmvKey);
	}
	...
	else
	{
		...
		if (ext->alg == IW_ENCODE_ALG_WEP)
		{
			...
			if(pAd->PortCfg.DesireSharedKey[real_keyIdx].KeyLen != 0)
			{
				pAd->SharedKey[real_keyIdx].KeyLen = pAd->PortCfg.DesireSharedKey[real_keyIdx].KeyLen;
				NdisMoveMemory(pAd->SharedKey[real_keyIdx].Key, pAd->PortCfg.DesireSharedKey[real_keyIdx].Key, pAd->SharedKey[real_keyIdx].KeyLen);

				pAd->SharedKey[real_keyIdx].CipherAlg = pAd->PortCfg.DesireSharedKey[real_keyIdx].CipherAlg;

				AsicAddSharedKeyEntry(pAd, 0, (UCHAR)real_keyIdx, pAd->SharedKey[real_keyIdx].CipherAlg, pAd->SharedKey[real_keyIdx].Key, NULL, NULL);
			}
			else
			{
				DBGPRINT(RT_DEBUG_TRACE, "%s(): Do add WEP key fail! keyIdx=0x%x!KeyLength=%d!\n", 
						__FUNCTION__, keyIdx, ext->key_len);
			}
		}
		else if((ext->alg == IW_ENCODE_ALG_TKIP) || (ext->alg == IW_ENCODE_ALG_CCMP))
		{
			...
			if (pAd->PortCfg.AuthMode >= Ndis802_11AuthModeWPA)
			{
				...
				{
					NdisMoveMemory(pRtmpKey->BSSID, BROADCAST_ADDR, 6);
				}

				RTMPWPAAddKeyProc(pAd, pRtmpKey);
			}
			else    // Old WEP stuff
			{
				RTMPWPAWepKeySanity(pAd, pRtmpKey);
			}
			DBGPRINT(RT_DEBUG_TRACE, "Set::OID_802_11_ADD_KEY (id=0x%x, Len=%d-byte)\n", pRtmpKey->KeyIndex, pRtmpKey->KeyLength);
		}
		else
		{
...
#define IW_PMKID_LEN 16
	...
	int found = 0;
#ifdef NATIVE_WPA_SUPPLICANT_SUPPORT
	DBGPRINT(RT_DEBUG_TRACE, "----> %s(): pmksa.cmd[%d]\n", __FUNCTION__, pmksa->cmd);
	if (pAd->PortCfg.bNativeWpa == TRUE)  // add by johnli
	{
	...
	else
	{
		...
		if (pmksa->cmd == IW_PMKSA_ADD)
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, "%s():Update IW_PMKSA_ADD, idx = %d\n", __FUNCTION__, CachedIdx);
			CachedIdx = (CachedIdx < PMKID_NO) ? CachedIdx : (pmksa->bssid.sa_data[5] % PMKID_NO);
			DBGPRINT(RT_DEBUG_TRACE, "%s():After adjust, IW_PMKSA_ADD, idx = %d\n", __FUNCTION__, CachedIdx);
		...
		{	
			...
			DBGPRINT(RT_DEBUG_TRACE, "%s():Remove IW_PMKSA_REMOVE, idx = %d\n", __FUNCTION__, CachedIdx);
			NdisZeroMemory(&pAd->PortCfg.SavedPMK[CachedIdx], sizeof(BSSID_INFO));
			pAd->PortCfg.SavedPMKNum--;
		}
	}
...
#endif
		DBGPRINT(RT_DEBUG_TRACE, "set WPA IE for Association Request!\n");
	} else {
		//The wpa ie length is zero, it means didn't enable wpa.
		DBGPRINT(RT_DEBUG_TRACE, "wrqu->data.length=%d! Stop wpa!\n", wrqu->data.length);
	}
	}
	DBGPRINT(RT_DEBUG_TRACE, "<---- %s\n", __FUNCTION__);
#endif // NATIVE_WPA_SUPPLICANT_SUPPORT //
...
static const iw_handler rt_handler[] =
{
	...
	/* Spy support */
	(iw_handler) NULL,					/* SIOCSIWSPY 0x10 */
	(iw_handler) NULL,					/* SIOCGIWSPY 0x11 */
	...
	(iw_handler) NULL,					/* SIOCSIWMLME   0x16 */
	...
	(iw_handler) rt_ioctl_siwnickn,				/* SIOCSIWNICKN 0x1c */
	(iw_handler) rt_ioctl_giwnickn,				/* SIOCGIWNICKN 0x1d */

	/* Other paramaters useful in 802.11 and some other devices */
	...
	(iw_handler) NULL,					/* SIOCGIWRETRY  0x29 */

	/* Encoding stuff */
	(iw_handler) rt_ioctl_siwencode,			/* SIOCSIWENCODE 0x2a */
	(iw_handler) rt_ioctl_giwencode,			/* SIOCGIWENCODE 0x2b */

	/* Power saving stuff */
	(iw_handler) NULL,					/* SIOCSIWPOWER  0x2c */
	(iw_handler) NULL,					/* SIOCGIWPOWER  0x2d */
	(iw_handler) NULL,					/* -- hole -- */	
	(iw_handler) NULL,					/* -- hole -- */
...
#if WIRELESS_EXT > 20
	.get_wireless_stats = (struct iw_statistics *) RT61_get_wireless_stats,
#endif
};
    ...
    IN  INT                 cmd)
{
    ...
    NDIS_802_11_ASSOCIATION_INFORMATION	AssociateState;
#endif      
//    USHORT                              TxTotalCnt;
	BOOLEAN								Cancelled;
    ...
    switch(cmd & 0x7FFF) {
        ...
        case OID_802_11_SSID:
            ...
            else
            {
				...
				{
					if((!INFRA_ON(pAdapter) || !pAdapter->MlmeAux.AutoReconnectSsidLen) && pSsid->SsidLength)
					{
					pAdapter->MlmeAux.AutoReconnectSsidLen = pSsid->SsidLength;
					NdisMoveMemory(pAdapter->MlmeAux.AutoReconnectSsid, pSsid->Ssid, pSsid->SsidLength);
        ...
        case RT_OID_802_11_STA_CONFIG:
            ...
            else
            {
                ...
                pAdapter->PortCfg.bEnableTxBurst = StaConfig.EnableTxBurst;
                ...
                DBGPRINT(RT_DEBUG_TRACE, "Set::RT_OID_802_11_SET_STA_CONFIG (Burst=%d, Protection=%d,ShortSlot=%d, Adhoc=%d\n",
                ...
                if (BssType == Ndis802_11IBSS) 
                {
                    if (pAdapter->PortCfg.BssType != BSS_ADHOC)
                    {
                        if (INFRA_ON(pAdapter))
						{
							...
							RTMPCancelTimer(&pAdapter->Mlme.LinkDownTimer, &Cancelled);
...
//				if (pAdapter->PortCfg.TxPowerDefault < pAdapter->PortCfg.TxPowerPercentage)
        ...
        case RT_OID_802_11_ADD_WPA:
            ...
            else
            {
                ...
                {   
                    NdisMoveMemory(pKey->BSSID, pAdapter->PortCfg.Bssid, 6);
        ...
        case OID_802_11_REMOVE_KEY:
			...
			/* carella modify: for wpa_supplicant remove key, but still leave the PMK cache */ //<--carella
#if 0
			if(pAdapter->PortCfg.AuthMode == Ndis802_11AuthModeWPA2)
			{
				int idx;

				for (idx = 0; idx < PMKID_NO; idx++)
				{
					NdisZeroMemory(pAdapter->PortCfg.SavedPMK[idx].BSSID, 6);
					NdisZeroMemory(pAdapter->PortCfg.SavedPMK[idx].PMKID, 16);
				}
			} //-->carella
#endif
                }
            ...
            else
            {
                ...
                else    // Old WEP stuff
                {
                    ...
                    else
                    {
                        ...
                        pAdapter->SharedKey[KeyIdx].KeyLen = (UCHAR) pKey->KeyLength;
                        NdisMoveMemory(pAdapter->SharedKey[KeyIdx].Key, &pKey->KeyMaterial, pKey->KeyLength);
	    ...
	    case OID_802_11_PMKID:
	        ...
	        /* carella modify: BSSIDInfoCount == 0 should flush the pmk cache info */
       ...
       break;	   
        case OID_802_11_STATE:
		Status  = -EINVAL;
		Status = copy_from_user(&AssociateState, wrq->u.data.pointer, wrq->u.data.length);
		if((pAdapter->Mlme.AssocMachine.CurrState == ASSOC_IDLE) &&
			(pAdapter->Mlme.CntlMachine.CurrState == CNTL_IDLE))
		{
			DBGPRINT(RT_DEBUG_TRACE, "Get:: Association Status is Success\n");
			AssociateState.AssociateResponseToMe = 1;
		}
		else
		{
			DBGPRINT(RT_DEBUG_TRACE, "Get:: Association Status is Failure\n");
			AssociateState.AssociateResponseToMe = 0;
		}

		Status = copy_to_user((void *)wrq->u.data.pointer, &AssociateState, sizeof(NDIS_802_11_ASSOCIATION_INFORMATION));
	break;
#endif
        default:
...
// in ExtRateIE which may be mis-treated as 802.11b AP by ZeroConfig
        ...
        case OID_802_11_BSSID:
            if (INFRA_ON(pAdapter) || ADHOC_ON(pAdapter))
            {
                Status = copy_to_user(wrq->u.data.pointer, &pAdapter->PortCfg.Bssid, sizeof(NDIS_802_11_MAC_ADDRESS));
        ...
        case RT_OID_802_11_RSSI_2:
			...
			{
				ulInfo = pAdapter->PortCfg.LastRssi2 - pAdapter->BbpRssiToDbmDelta;
				wrq->u.data.length = sizeof(ulInfo);
				Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
				DBGPRINT(RT_DEBUG_TRACE, "Query::RT_OID_802_11_RSSI_2(=%d)\n", ulInfo);
			}
			else
				Status = -EOPNOTSUPP;
			break;
        ...
        case OID_802_11_NETWORK_TYPES_SUPPORTED:	
			if ((pAdapter->RfIcType == RFIC_5225) || (pAdapter->RfIcType == RFIC_5325))
			{
	...
	IN	INT					cmd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,"=====Ioctl Cmd=%x =====\n",cmd);
	...
	{			
		...
		case SIOCGIWNAME:
			...
			strcpy(wrq->u.name, "RT61 Wireless");
			break;
...
//#if WPA_SUPPLICANT_SUPPORT
	...
	//NdisMoveMemory(&pmkid_list.CandidateList[0].BSSID, pAd->PortCfg.Bssid, 6);
    ...
    IN  PUCHAR          arg)
{
	...
	BlockChannelBitMap=simple_strtoul(arg, 0, 10);

	pAdapter->PortCfg.BlockChannelBitMap=BlockChannelBitMap;

	DBGPRINT(RT_DEBUG_TRACE, "Set_BlockChannelBitMap_Proc::(BlockChannelBitMap=%d)\n", pAdapter->PortCfg.BlockChannelBitMap);
...
INT	Set_ScanTabClean_Proc(
	...
	IN	PUCHAR			arg)
{
	DBGPRINT(RT_DEBUG_TRACE, "==> Set_ScanTabClean_Proc *******************\n");

        pAd->scanTabCleanTime = simple_strtol(arg, 0, 10);   

	 if(pAd->scanTabCleanTime%2==0)
	 	pAd->scanTabCleanTime=pAd->scanTabCleanTime+1;
		

	DBGPRINT(RT_DEBUG_TRACE, "<== Set_ScanTabClean_Proc(%d)\n", pAd->scanTabCleanTime);
	...
	IN	PUCHAR			arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, "==> Set_ShowScanTab_Proc ,time is %u*******************\n",jiffies);

	for(i=0;i<pAd->ScanTab.BssNr;i++)
	{
		printk("%03d: MAC:%02x:%02x:%02x:%02x:%02x:%02x  Channel:%02d Rssi:%03d Time:%u SSID:%s %d\n",i,
			pAd->ScanTab.BssEntry[i].Bssid[0],pAd->ScanTab.BssEntry[i].Bssid[1],
			pAd->ScanTab.BssEntry[i].Bssid[2],pAd->ScanTab.BssEntry[i].Bssid[3],
			pAd->ScanTab.BssEntry[i].Bssid[4],pAd->ScanTab.BssEntry[i].Bssid[5],
			pAd->ScanTab.BssEntry[i].Channel,
			pAd->ScanTab.BssEntry[i].Rssi-pAd->BbpRssiToDbmDelta,pAd->ScanTab.BssEntry[i].LastBeaconRxTime,pAd->ScanTab.BssEntry[i].Ssid,
...
INT	Set_R17D_Proc(
	...
	IN	PUCHAR			arg)
{
	DBGPRINT(RT_DEBUG_TRACE, "==> Set_R17D_Proc *******************\n");

        pAd->BbpTuning.R17LowerUpperSelect = simple_strtol(arg, 0, 10);   

	DBGPRINT(RT_DEBUG_TRACE, "<== Set_R17D_Proc(%d)\n", pAd->BbpTuning.R17LowerUpperSelect);
    ...
    IN  PUCHAR          arg)
{
	...
	printk("mgmt ring CurTxIndex:%d NextDmaDoneIndex:%d \n",pAdapter->MgmtRing.CurTxIndex
		,pAdapter->MgmtRing.NextTxDmaDoneIndex);
...
#endif
		printk("%d: Owner:%d bWaitingDmaDoneInt:%d  pTxD->BufCount:%d \n",i,pTxD->Owner,pTxD->bWaitingDmaDoneInt,pTxD->BufCount);
		printk("       BufLen:%d %d %d %d %d \n",pTxD->BufLen0,pTxD->BufLen1,pTxD->BufLen2,pTxD->BufLen3,pTxD->BufLen4);

		//printk("pAdapter->MgmtRing.Cell[pAdapter->MgmtRing.CurTxIndex].pSkb");
#if 0
		if(pAdapter->MgmtRing.Cell[i].pSkb!=NULL)
		dumpPkt(pAdapter->MgmtRing.Cell[i].pSkb->data,pAdapter->MgmtRing.Cell[i].pSkb->len );
#endif
#ifdef BIG_ENDIAN
    RTMPDescriptorEndianChange((PUCHAR)pTxD, TYPE_TXD);
    *pDestTxD = TxD;
	...
	IN	PUCHAR			arg)
{
	...
	if ((APSDPsmBit == 0) || (APSDPsmBit == 1))
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, "IF(ra%d) Set_APSDPsm_Proc::(bAPSDForcePowerSave=%d)\n", 
    ...
    IN  PUCHAR          arg)
{
    ...
    arg[strlen(arg)] = '\0'; // make STA can process .$^& for WPAPSK input 
...

			{
				...
				if (macAddr == (HW_DEBUG_SETTING_BASE + 4))
				{
                    ...
                    else
                    {
                        ...
                        RTMP_BBP_IO_WRITE8_BY_REG_ID(pAdapter, 17, R17);
	...
	IN	struct iwreq	*wrq)
{
   ...
   if (INFRA_ON(pAd) || ADHOC_ON(pAd))
   	...
   	linkstate=pAd->LinkDownReason;

    memset(msg, 0x00, 1600);
    sprintf(msg, "\n");

    sprintf(msg+strlen(msg), "Link State                      = %d\n", linkstate);
    sprintf(msg+strlen(msg), "Max TxRate                      = %d\n", pAd->PortCfg.MaxTxRate);
	...
	IN	struct iwreq	*wrq)
{
    ...
    for(i=0;i<MAX_LEN_OF_BSS_TABLE;i++)
    {
		...
		sprintf(msg+strlen(msg),"%-36s", strSsid);      
	...
	IN	PUCHAR			arg)
{
	...
	{						
		...
		if (atemode == ATE_TXFRAME)
		{
			...
			{
				PRTMP_DMABUF    pDmaBuf = &pAd->TxRing[QID_AC_BE].Cell[i].DmaBuf;
	...
	{						
		...
		for (i = 0; (i < TX_RING_SIZE) && (i < pAd->ate.TxCount); i++)
		{
			...
			PRTMP_DMABUF    pDmaBuf = &pAd->TxRing[QID_AC_BE].Cell[pTxRing->CurTxIndex].DmaBuf;
			...
			memcpy(&pDestBufVA[4], pAd->ate.Addr1, MAC_ADDR_LEN);
			memcpy(&pDestBufVA[10], pAd->ate.Addr2, MAC_ADDR_LEN);
			memcpy(&pDestBufVA[16], pAd->ate.Addr3, MAC_ADDR_LEN);
			...
			*pDestTxD = TxD;
			pTxD = pDestTxD;
#endif			
			pAd->TxRing[QID_AC_BE].Cell[pTxRing->CurTxIndex].pSkb = pSkb;
			pAd->TxRing[QID_AC_BE].Cell[pTxRing->CurTxIndex].pNextSkb = NULL;
			RTMPWriteTxDescriptor(pAd, pTxD, CIPHER_NONE, 0, 0, FALSE, FALSE, FALSE, SHORT_RETRY, 
				IFS_BACKOFF, pAd->ate.TxRate, pAd->ate.TxLength, QID_AC_BE, 0, FALSE, FALSE, FALSE);
	...
	IN	PUCHAR			arg)
{
	...
	if (pAdapter->ate.TxPower > 31)  
	{
		...
		if (pAdapter->ate.TxPower <= 36)
			Bbp94 = BBPR94_DEFAULT + (UCHAR) (pAdapter->ate.TxPower - 31);		
		pAdapter->ate.TxPower = (CHAR)R3;
	}
