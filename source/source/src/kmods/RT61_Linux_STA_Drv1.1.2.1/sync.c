    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
	...
	// reset all the timers, remove all timer to stop all process.
	RTMPCancelTimer(&pAd->MlmeAux.ScanTimer, &Cancelled);
	RTMPCancelTimer(&pAd->MlmeAux.BeaconTimer, &Cancelled);
...
DBGPRINT(RT_DEBUG_TRACE,"MlmeJoinReqAction - PortCfg.Ssid[%d]=%s... MlmeAux.Ssid[%d]=%s\n",
	...
	if(pBss->SsidLen>0&&pBss->Ssid!=NULL&&pBss->Ssid[0]!=0)
	{
	...
	else
	{
		...
		//do nothing, don't allow the SSID which is derived from ScanTable is empty	
	}
	...
	//Send probe request when wait for joining beacon
	//
	// send probe request
	//
	pOutBuffer = kmalloc(MAX_LEN_OF_MLME_BUFFER, MEM_ALLOC_FLAG);	//Get an unused nonpaged memory 
	if (pOutBuffer != NULL)
	{
		...
		//We will create a new frame whose header is filled by the following function MgtMacHeaderInit.
	...
	//2007/11/09:KH add to avoid the station waiting beacon forever.
	RTMPSetTimer(pAd, &pAd->MlmeAux.BeaconTimer, JOIN_TIMEOUT);
	//2007/11/09:KH add to avoid the station waiting beacon forever.(End)
...
//	UCHAR		  LenVIE;
    ...
    if (PeerBeaconAndProbeRspSanity(pAd, 
                                ...
                                pVIE)) 
    {
		//--> BensonLiu modify 2007-11-13 startBEACON from desired BSS/IBSS
		// Disqualify 11b only adhoc when we are in 11g only adhoc mode
		if ((BssType == BSS_ADHOC) && (pAd->PortCfg.AdhocMode == 2) && ((SupRateLen+ExtRateLen)< 12) && (Channel <= 14))		
			return;
		//<-- BensonLiu modify 2007-11-13 end
		...
		if (MAC_ADDR_EQUAL(pAd->MlmeAux.Bssid, Bssid))
        {
            ...
            if (pAd->RxAnt.EvaluatePeriod == 0)
			{
	            ...
	            {
	            	RealRssi = ConvertToRssi(pAd, Elem->Signal, RSSI_NO_2);
	            	pAd->PortCfg.LastRssi2  = RealRssi + pAd->BbpRssiToDbmDelta;
	           		pAd->PortCfg.AvgRssi2  = pAd->PortCfg.LastRssi2;
				...
				if(SsidLen==0)//hidden SSID
				{
			           	...
			           	pAd->MlmeAux.SsidLen = pAd->PortCfg.SsidLen;
			                ...
			                pAd->MlmeAux.SsidLen =pAd->PortCfg.LastSsidLen;
                                      ...
                                      //do nothing, don't allow the SSID is empty except we set SSID by *.dat or iwpriv
			...
			DBGPRINT(RT_DEBUG_INFO, "SYNC - PeerBeaconAtJoinAction "
					 "Addr2=%02x:%02x:%02x:%02x:%02x:%02x, BSSID=%02x:%02x:%02x:%02x:%02x:%02x, ssid='%.*s' \n", 
                                ...
                                pVIE)) 
    {
		...
		if ((! is_my_bssid) && ADHOC_ON(pAd) && (BssType == BSS_ADHOC))
		{
			...
			for (i = 0; i < 6; i++)
			{
				...
				else if (Bssid[i] < pAd->PortCfg.Bssid[i])
					break;
			}
		}
        ...
        if (is_my_bssid)
        {
			...
			if ((INFRA_ON(pAd) && (pAd->PortCfg.Psm == PWR_SAVE)) || (pAd->PortCfg.bAPSDForcePowerSave))
            {
            	UCHAR FreeNumber;
...
//For A band 
	...
	if (pAd->MlmeAux.Channel == 0) 
	{
		...
		{
			UINT i=0;
			DBGPRINT(RT_DEBUG_TRACE, "Scan Outcome %u %u time:%u (%s)\n",pAd->ScanTab.BssNr,pAd->ScanTab.BssOverlapNr,jiffies,pAd->MlmeAux.AutoReconnectSsid);
			for(i=0;i<pAd->ScanTab.BssNr;i++)
			{
				if(SSID_EQUAL(pAd->ScanTab.BssEntry[i].Ssid,pAd->ScanTab.BssEntry[i].SsidLen,pAd->MlmeAux.AutoReconnectSsid,pAd->MlmeAux.AutoReconnectSsidLen))
				{
				DBGPRINT(RT_DEBUG_TRACE,"%03d: MAC:%02x:%02x:%02x:%02x:%02x:%02x  Channel:%02d Rssi:%03d Time:%u SSID:%s %d\n",i,
	...
	else 
	{
		...
		if (pAd->MlmeAux.Channel <= 14)
		{
        	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, 17, pAd->BbpTuning.R17LowerBoundG);
            //
		...
		else
		{
			...
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, 17, pAd->BbpTuning.R17LowerBoundA);        
        	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R62, 0x04);
		}
    ...
    IN UCHAR channel)
{
	...
	{
		ULONG b=1;
	//	printk(" %x %x %x %x %x\n",channel,i,b<<i,pAd->PortCfg.BlockChannelBitMap,(b<<i) & pAd->PortCfg.BlockChannelBitMap);
		if ((b<<i) & pAd->PortCfg.BlockChannelBitMap)
			continue;
		
		next_channel = pAd->ChannelList[i].Channel;
		break;
	}    
