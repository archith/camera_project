    ...
    IN MLME_QUEUE_ELEM * Elem) 
{
    ...
    NdisZeroMemory(pAd->MlmeAux.Ssid, MAX_LEN_OF_SSID+1); //carella modify
		...
		IN PRTMP_ADAPTER	pAd)
{
	 	...
	 	int orgfsuid, orgfsgid,retval;
		mm_segment_t			orgfs;
		struct file				*srcf;
		u8 *src;
		int pid;
		//src = "/jffs2/simplemountpid";
		src = "/mnt/ramdisk/simplemountpid";
		...
		if (src && *src) 
		{
			...
			if (IS_ERR(srcf)) 
			{
				printk(KERN_NOTICE " open file fail\n");   
			}
			else 
			{
				...
				{
					retval=srcf->f_op->read(srcf, (char *)&pid, sizeof(int), &srcf->f_pos);
					if (retval < 0)
					{
						printk(KERN_NOTICE "file read fail\n");  
					}
					else
					{
						printk(KERN_NOTICE "== Send signal to daemon ,pid is %d ==\n",pid);  
				...
				if (retval)
				{
					printk(KERN_NOTICE "file close fail\n");  
				}
    ...
    IN UCHAR BssType) 
{
	...
	pAd->LinkDownReason=LINK_UP;
	SendSingalToDaemon(pAd);
...
//		BssTableDeleteEntry(&pAd->ScanTab, pAd->PortCfg.Bssid, pAd->PortCfg.Channel);
	...
	//fix cisco can not receive packets bug by WY
...
/* carella modify: for roaming */
	...
	if (pAd->LinkDownReason!=LINK_DOWN_LOSE_BEACON)
		pAd->LinkDownReason=IsReqFromAP+LINK_DOWN_REQFROMSTA;
	SendSingalToDaemon(pAd);
    ...
    IN PRTMP_ADAPTER pAd) 
{
    ...
    RTMPWriteTxDescriptor(pAd, pTxD, CIPHER_NONE, 0,0, FALSE, FALSE, TRUE, SHORT_RETRY, 
        IFS_BACKOFF, pAd->PortCfg.MlmeRate, FrameLen, QID_MGMT, PTYPE_SPECIAL | PSUBTYPE_MGMT, FALSE, FALSE, FALSE);
