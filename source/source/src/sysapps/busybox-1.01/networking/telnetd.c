/* $Id: telnetd.c,v 1.1.1.1 2005/12/08 08:38:23 jedy Exp $
 *
...
telnetd_main(int argc, char **argv)
{
	...
	for (;;) {
		...
		switch (c) {
			...
			case 'l':
				...
				break;
			case 't':
				g_timeout_second = atoi(optarg) * 60;
				break;
#ifndef CONFIG_FEATURE_TELNETD_INETD
			case 'p':
				portnbr = atoi(optarg);
	...
	do {
		...
		while (ts) { /* For all sessions...  */
			...
			if(g_timeout_second != -1 && current_info.uptime - ts->info.uptime > g_timeout_second)
			{
				char buf[512];
				fprintf(stderr, "current_info.uptime=%d, tv=%d\n", (int)current_info.uptime, (int)ts->info.uptime);
				sprintf(buf, "TIMEOUT, No commands within %d seconds!!!\r\n", g_timeout_second);
				w = write(ts->sockfd, buf, strlen(buf));
				free_session(ts);
				ts = next;
				continue;
			}
