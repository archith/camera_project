...
/***** Add by Yen*****/
/***** Add remote IP address to the http header, *****/
/***** to tell web server the real client address *****/
int add_remote_ip_to_header(CLI *c)
{
	...
	//fprintf(stderr, "%s", c->ssl_buff);
	header_end = (int)strstr(c->ssl_buff, "\r\n\r\n") - (int)c->ssl_buff;
	header_end += 2;

	post_len = strlen(c->ssl_buff+header_end);
	post_buf = (char*)malloc(post_len+1);
	strcpy(post_buf, c->ssl_buff+header_end);
	*(post_buf+post_len) = '\0';
	//fprintf(stderr, "post: %s", post_buf);

	s_ntop(real_remote_addr, &c->peer_addr.addr[0]);
	colon = strchr(real_remote_addr, ':');
	real_remote_addr[(int)colon - (int)real_remote_addr] = '\0';
	strcat(real_remote_addr, "\r\n");
	sprintf(c->ssl_buff+header_end, "StunnelRemoteIP: %s", real_remote_addr);
	//fprintf(stderr, "real add: %s", real_remote_addr);
	strcat(c->ssl_buff, post_buf);
	c->ssl_ptr = strlen(c->ssl_buff);

	free(post_buf);
	//fprintf(stderr, "After add real ip:\n%s", c->ssl_buff);
