				      ...
				      void *ctx, char *custom)
{
	...
	else if (strncmp(custom, "ASSOCINFO_ReqIEs=", 17) == 0)
	{
		...
		spos = custom + 17;
        //get IE's length
		/* carella modify: VARIE need update for WPA2 */ //<--carella
		//bytes = strlen(spos);
		bytes = drv->assoc_req_ies_len + drv->assoc_resp_ies_len; //-->carella
		if (!bytes)
			return;
...
{
//	char buf[8192];
	char buf[RT_MAX_802_11_BSSID_LIST_EX];
	int left;
...
#if 0  // some cases, timeout is too short ??
	eloop_register_timeout(4, 0, wpa_driver_ralink_scan_timeout, drv, drv->ctx);
#else
	eloop_register_timeout(6, 0, wpa_driver_ralink_scan_timeout, drv, drv->ctx);
#endif
				     ...
				     size_t max_size)
{
	...
	if (ret = ioctl(drv->ioctl_sock, RT_PRIV_IOCTL, &iwr) < 0)
	{
		os_free(buf);
		return -1;
	}
