...
#if 1
	if((wpa_s->wpa->proto == WPA_PROTO_RSN &&  wpa_s->wpa->pmksa == NULL && wpa_s->wpa_state < WPA_ASSOCIATED) || //<--carella modify 2007/11/27: for WPA2 pmk cached and wait AP send EAPOL-Key.
		(wpa_s->wpa->proto != WPA_PROTO_RSN && wpa_s->wpa_state < WPA_ASSOCIATED)){
		printf("<--- carella --->\n");
		return; //-->carella
	}
#else
	if(wpa_s->wpa->proto == WPA_PROTO_RSN &&  wpa_s->wpa->pmksa == NULL && wpa_s->wpa_state < WPA_ASSOCIATED)
	{
		printf("!!carella\n");
		return; //-->carella
	}
