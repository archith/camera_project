...
#define A_SSL_DEFAULT_CIPHER_LIST	"RC4:SHA:AES128:DHE:RSA:ADH" /* low priority for RC4 */
...
{
	SSL_CTX *ssl;
	char	*cipher_list=A_SSL_DEFAULT_CIPHER_LIST;
	...
	SSL_CTX_set_info_callback(ssl, ssl_info_cb);
        ...
        if (!SSL_CTX_set_cipher_list(ssl, cipher_list)) {
		wpa_printf(MSG_ERROR, "SSL: failed to set cipher_list");
	}
