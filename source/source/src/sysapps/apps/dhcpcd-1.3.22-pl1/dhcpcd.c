...
time_t        NotifyTimeOut  =	NOTIFY_WAIT_TIME;    //Added for Auto-IP
char	      bRunScriptAndQuit = 0;					
...
/*
	...
	bAutoIPEnable = 1   --> Auto IP Enable	
	bAutoIPEnable = 0   --> Auto IP Disable	
	
	default value is 0
	use -X argument to set bAutoIPEnable to 1
	ex: dhcpcd -X
*/
#ifdef _SUPPORT_AUTOIP_
char	      bAutoIPEnable = 0;	
#endif
...
prgs: switch ( argc[i][s] )
	{	
	  ...
	  case 't':
	    ...
	    if ( TimeOut > 0 ) break;
	    goto usage;
	  case 'p':	//Added for Auto-IP to pass the wait time before changing the IP address
	    i++;
	    if ( argc[i] )
	      NotifyTimeOut=atol(argc[i++]);
	    else
	      goto usage;
	    s=1;
	    if ( NotifyTimeOut >= 0 ) break;
	    goto usage;
	  case 'w':
	    if ( argc[i][s+1] ) goto usage;
            i++;
	  ...
	  case 'T':
	    ...
	    TestCase=1;
	    goto prgs;
#if 0				// Allen: 還不確定這段CODE是否要加進來		
	  case 'e':     //Stop and Start the Script files and quit
	    bRunScriptAndQuit= 1; //Not quitting from here because may want
	    			 //behave differently for different interface. Should parse all the
	    			// parameters. So, would not run if there is a wrong parameter
	    s++;
	    break;
#endif		
	  case 'l':
...
#ifdef _SUPPORT_AUTOIP_
	  case 'X':	//	add for Auto IP : disable Auto IP
	    		s++;
			bAutoIPEnable = 1;
	    		goto prgs;
#endif
...
#ifdef _SUPPORT_AUTOIP_
	if( bAutoIPEnable == 1) //Added for AutoIP
	{
  		do
    			if ( (currState=(void *(*)())currState()) == NULL ) exit(1);	
	  	while (( currState != &dhcpBound ) && ( currState != &dhcpWaitInAutoIP));
