...
extern unsigned char    bAutoConfiguredIP; //added for autoIP
...
int sig;
{
  ...
  if ( sig == SIGALRM )
...
#ifdef _SUPPORT_AUTOIP_
		  { // add for auto ip
		    	if( ( currState == &dhcpInit ) && (bAutoIPEnable ==1 ))
		      		siglongjmp(env,5);  
		  	else
#endif
	...
	{
#ifdef _SUPPORT_AUTOIP_
	  if( (bAutoConfiguredIP == 0) ||( bAutoIPEnable == 0) )
	  {
#endif
	  	dhcpRelease();
...
void signalSetup()
{
  ...
  /* Do not need this. This was causing sleep to fail. */ 
#ifndef _SUPPORT_AUTOIP_
