...
//#define SHOW_CGI_STDERR
//#define _LIGHTTPD_DEBUG_
#ifdef _LIGHTTPD_DEBUG_
#include <stdio.h>
#else
#endif

#define	MAX_CGI_NUM	15	/* MODIFIED: max cgi child number*/
...
typedef struct {
	...
	/* internal */
	buffer *pathinfo;
	/* MODIFIED: User-Agent: (video handler) */
	buffer *useragent;
	/* MODIFIED: sessionID: (session id for Linksys cgi) */
	buffer *session_id;
	/* MODIFIED: StunnelRemoteIP: (real client ip while using stunnel) */
	buffer *https_client;
	/* MODIFIED: X-SESSIONCOOKIE: (rtp/rtsp) */
	buffer *x_sessioncookie;
} request;
...
typedef struct {
	...
	buffer *authed_user;
	buffer *cgi_auth;// MODIFIED: for cgi environment variable CGI_AUTHORIZATION
	...
	int in_video_handler;	/* MODIFIED: 1= video cgi handled */

	int server_busy; 	// MODIFIED

	int camera_api_client;   // MODIFIED: 1 = request comes from camera_ap port

	int passthrough_client;	// MODIFIED: 1 = request comes from passthrough port
	int passthrough_idx;	// MODIFIED: index in passthrough_lock;
	int passthrough_max;	// MODIFIED: 1 = reaching max passthrough client

	int cgi_max_reached;	// MODIFIED: 1 = reaching max cgi child number

	int audio_upload;	// MODIFIED: 1 = request is audio upload

	buffer* authinfo[3];	// MODIFIED: auth info, array 0 for cookie, 
				// array 1 for query string, array 2 for request header

	int upgrade_fw;		// MODIFIED: about to upgrade FW
	int discard_content;	// MODIFIED: discard POST content
...
typedef struct server {
	...
	void* adm_lock_status;	// MODIFIED
	void* passthrough_lock;	// MODIFIED
	int cgi_number;	// MODIFIED
	int fw_upgrading;	// MODIFIED
